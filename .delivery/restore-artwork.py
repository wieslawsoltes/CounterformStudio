"""Restore a digest-verified source delta without overwriting divergent files."""
...
EXPECTED = '8ba7e7ec7d2ecf3dcc13aca1edc2e729efafa606beac4ec63b85526f82ed33d6'
BASE = '722abe3cab4c99436963f11d32192d479d1704d2'
...
def main():
    parts = sorted((ROOT / '.delivery').glob('artwork.*.xzpart'))
    if [p.name for p in parts] != [f'artwork.{i:02}.xzpart' for i in range(6)]:
        raise ValueError('Unexpected source part names')
    ...
    if manifest['format'] != 1 or manifest['baseCommit'] != BASE or len(manifest['records']) != 83:
    ...
    # Verify every input and result before writing any editable source.
