...
export const ribbonTabs=[
 ...
 {id:'design',label:'Design',groups:[{id:'nodes',label:'Nodes',commands:['node.smooth','node.corner','outline.extrema','node.start']},{id:'paths',label:'Contours',commands:['outline.overlap','outline.join','outline.open','outline.close','outline.clean']},{id:'curves',label:'Segments',commands:['outline.lines','outline.curves','outline.reverse','outline.winding']},{id:'transform',label:'Transform',commands:['tool.rotate','tool.scale','tool.slant','outline.transform','outline.mirrorX','outline.round']}]},
 {id:'spacing',label:'Spacing',groups:[{id:'metrics',label:'Metrics',commands:['font.info','glyph.center','kern.pair']},{id:'align',label:'Align & distribute',commands:['node.alignX','node.alignY','node.distributeX','node.distributeY']},{id:'kern',label:'Kerning',commands:['view.kerning','kern.groups']}]},
 {id:'masters',label:'Masters',groups:[{id:'masters',label:'Design space',commands:['view.masters','master.add','axis.add','master.compatibility','master.instance']},{id:'components',label:'Components',commands:['glyph.component','glyph.decompose']}]},
 {id:'opentype',label:'OpenType',groups:[{id:'compile',label:'Compilation',commands:['view.features','features.apply','font.validate','font.tables']},{id:'color',label:'Color & attachment',commands:['color.edit','glyph.anchor','glyph.guides']},{id:'export',label:'Production',commands:['file.export','glyph.svg']}]},
 {id:'workspace',label:'Workspace',groups:[{id:'view',label:'Canvas',commands:['view.fit','view.grid','view.fill','view.preview','view.snap']},{id:'layout',label:'Layout',commands:['layout.undo','layout.redo','view.theme','view.fullscreen']},{id:'tools',label:'Tools',commands:['commands.palette','commands.bindings','automation.recipe','app.toolHelp','app.about']}]}];
export function menuDefinitions(registry){
 const definitions=[
  {label:'File',items:['file.new','file.open','file.recent',null,'file.save','file.export','glyph.svg',null,'file.demo']},
  ...
  {label:'View',items:['view.glyph','view.catalog','view.kerning','view.features','view.notes',null,'view.fit','view.zoomIn','view.zoomOut',null,'view.grid','view.fill','view.nodes','view.guides','view.preview','view.snap',null,'view.theme','view.fullscreen']},
  {label:'Font',items:['font.info','glyph.new','master.add','axis.add','master.instance','master.compatibility',null,'kern.pair','kern.groups','color.edit','features.apply',null,'font.validate','font.tables']},
  {label:'Glyph',items:['glyph.new','glyph.duplicate','glyph.delete','glyph.previous','glyph.next',null,'glyph.center','glyph.anchor','glyph.guides','glyph.clearGuides','glyph.lock',null,'glyph.component','glyph.decompose']},
  {label:'Contour',items:['node.smooth','node.corner','node.start','outline.extrema',null,'outline.join','outline.open','outline.close','outline.reverse','outline.winding',null,'outline.lines','outline.curves','outline.clean','outline.round',null,'outline.overlap','outline.union','outline.difference','outline.intersect','outline.xor','outline.stroke',null,'outline.transform','outline.mirrorX','outline.mirrorY','node.alignX','node.alignY','node.distributeX','node.distributeY']},
  {label:'Tools',items:[...tools.map(t=>'tool.'+t.id),null,'tool.options','automation.recipe','compute.verify']},
  {label:'Window',items:['view.inspector','view.masters','view.proof','view.output',null,'layout.undo','layout.redo']},
...
export function createToolRail(app){
 ...
 const ac=new AbortController();app.disposables.push(()=>ac.abort());app.toolRail.addEventListener('keydown',e=>{let i=all.indexOf(document.activeElement);if(i<0)return;
  const offsets={ArrowRight:1,ArrowLeft:-1,ArrowDown:2,ArrowUp:-2};let next=e.key==='Home'?0:e.key==='End'?all.length-1:Object.hasOwn(offsets,e.key)?(i+offsets[e.key]+all.length)%all.length:null;
