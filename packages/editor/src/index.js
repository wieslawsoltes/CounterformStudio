...
export class GlyphEditor {
    ...
    refresh() {
        ...
        this.renderer.setScene({ artwork:l.artwork||[], colorGlyphId:this.glyphId,hasColorPaint:!!this.glyph.colorPaint||!!this.glyph.bitmaps?.length,documentId:this.doc.data.id,revision:this.doc.revision,masterId:this.masterId,colorLayers, contours: this.doc.resolve(this.glyphId, this.masterId), editable: l.contours, advanceWidth: l.advanceWidth, metrics: {...this.doc.info,...this.doc.data.masters.find(m=>m.id===this.masterId)?.metrics}, anchors: l.anchors, guides: l.guides });
        this.reindex();
