...
export interface FontSource {paletteTypes?:number[];paletteLabels?:string[];paletteEntryLabels?:string[];originalFont?:{format:1;filename:string;bytes:string;sha256:string;sourceSha256:string;structureSha256:string;info:Record<string,unknown>};format:'counterform';version:1;id:string;info:FontInfo;axes:Axis[];masters:Master[];instances:Instance[];glyphs:Glyph[];kerning:Record<string,Record<string,number>>;groups:Record<string,string[]>;features:string;palettes:string[][];notes:string;richNotes?:unknown;importInfo:unknown;}
