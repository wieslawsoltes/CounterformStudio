...
function sourceText(source,structural=false){const data={...source};delete data.originalFont;delete data.id;delete data.notes;delete data.richNotes;if(structural){data.info={...data.info};for(const key of editable)delete data.info[key];}return JSON.stringify(data,(_,v)=>v&&typeof v==='object'&&!Array.isArray(v)?Object.fromEntries(Object.keys(v).sort().map(k=>[k,v[k]])):v);}
