...
export function createRibbon(registry, tabs, { theme = 'light' } = {}) { registerRibbon(); const ribbon = document.createElement('ribbon-web'); ribbon.model = { id: 'counterform-ribbon', title: 'Counterform Studio', theme, layout: 'simplified', tabs: tabs.map(t => ({ id: t.id, header: t.label, groups: t.groups.map(g => ({ id: g.id, header: g.label, items: g.commands.map(id => { const c = registry.commands.get(id); return { id, type: 'button', label: c?.label || id, tooltip: c?.description || c?.label || id, command: { CanExecute: () => registry.canExecute(id), Execute: () => registry.run(id).catch(() => { }) }, size: 'small' }; }) })) })), quickAccessToolbar: [], backstage: ['file.new', 'file.open', 'file.save', 'file.export'].filter(id => registry.commands.has(id)).map(id => ({ id, label: registry.commands.get(id).label, command: () => registry.run(id) })) }; return ribbon; }
