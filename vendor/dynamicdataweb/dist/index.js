    ...
    return source.subscribe({ next: (changes) => {
      try {
        ...
        if (typeof target === "function") target(items, changes);
          ...
          const clear2 = collection.clear ?? collection.Clear;
          const add = collection.addRange ?? collection.AddRange ?? collection.addOrUpdate ?? collection.AddOrUpdate;
          if (typeof clear2 !== "function" || typeof add !== "function") throw new TypeError("Binding collection updater requires Clear/clear and AddRange/addRange or AddOrUpdate/addOrUpdate.");
          clear2.call(collection);
...
// src/lifecycle.js
import { Observable as Observable3, Subject as Subject2, ReplaySubject, Subscription as Subscription3, asyncScheduler, from, of, isObservable as isObservable2, filter as rxFilter, map, distinctUntilChanged, debounceTime, bufferTime, skip, finalize, combineLatest, fromEvent, share } from "rxjs";
...
var notificationStreams = /* @__PURE__ */ new WeakMap();
...
var allProperties = (name) => name == null || name === "";
...
function nativePropertyEvents(item) {
  for (const name of ["PropertyChanged", "Changed", "propertyChanged", "changed"]) {
    ...
    if (isObservable2(stream)) return stream;
  }
}
...
function propertyEvents(item) {
  ...
  if (!stream) {
    stream = new Observable3((observer) => {
...
function changedProperty(item, property, notifyInitial = true) {
  if (typeof property === "string" && property.includes(".")) {
    ...
    return new Observable3((observer) => {
      const resources = new Subscription3(), connections = /* @__PURE__ */ new Map();
      let currentValue;
      resources.add(() => {
      ...
      const rewire = () => {
        ...
        for (const [target, names] of desired) {
          ...
          const entry = { names, subscription: new Subscription3() };
...
function whenAnyPropertyChanged(itemOrChanges, ...propertyNames) {
  const observe = (item) => propertyEvents(item).pipe(rxFilter((event) => !propertyNames.length || allProperties(event.propertyName) || propertyNames.includes(event.propertyName)), map(() => item));
