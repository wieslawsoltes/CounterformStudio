...
const cs = (changes, kind = "cache") => new import_core.ChangeSet(changes, kind);
...
function tracker(hooks = {}) {
  const state = { kind: "cache", entries: [], byKey: /* @__PURE__ */ new Map(), disposed: false };
  const add = (item, key, index = state.entries.length, previous, prior) => {
    if (state.disposed) return;
    if (prior) prior.prior = void 0;
    const entry = { item, key, prior, active: true, subscription: new import_rxjs.Subscription() };
    if (state.kind === "cache") {
  ...
  state.apply = (changes) => {
    ...
    for (const change of getChanges(changes)) {
      ...
      if (reason === "add") add(change.current, change.key, change.currentIndex >= 0 ? change.currentIndex : state.entries.length);
      ...
      } else if (reason === "update" || reason === "replace") {
        const old = at(change);
        ...
        const range = change.range ?? { items: change.current ?? [], index: change.currentIndex };
        if (range.index >= 0) {
          for (let i = 0; i < range.items.length; i++) remove(state.entries[range.index]);
        } else for (const item of range.items) remove(state.entries.find((entry) => entry.item === item));
      } else if (reason === "clear") {
        for (const entry of [...state.entries]) remove(entry);
      } else if (reason === "refresh") {
        const entry = at(change);
        if (entry) hooks.refresh?.(entry);
      } else if (reason === "moved" || reason === "move") {
...
const notificationStreams = /* @__PURE__ */ new WeakMap();
...
function propertySubject(item) {
  ...
  if (!subject) notifications.set(target, subject = new import_rxjs.Subject());
  return subject;
}
function nativePropertyEvents(item) {
  for (const name of ["PropertyChanged", "Changed", "propertyChanged", "changed"]) {
    ...
    if ((0, import_rxjs.isObservable)(stream)) return stream;
  }
}
function normalizePropertyEvent(item, event) {
  if (event == null || typeof event !== "object") return { sender: item, propertyName: event, value: void 0, previous: void 0 };
  return {
...
function propertyEvents(item) {
  ...
  if (!stream) {
    stream = new import_rxjs.Observable((observer) => {
      const resources = new import_rxjs.Subscription();
...
function readProperty(item, property) {
  ...
  if (typeof property === "symbol") return item?.[property];
  return String(property).split(".").reduce((value, part) => value?.[part], item);
}
function notifyPropertyChanged(item, propertyName, previous) {
  propertySubject(item).next({ sender: item, propertyName, value: readProperty(item, propertyName), previous });
}
...
const observableObject = createObservableObject;
function changedProperty(item, property, notifyInitial = true) {
  if (typeof property === "string" && property.includes(".")) {
    const parts = property.split(".");
    return new import_rxjs.Observable((observer) => {
      const resources = new import_rxjs.Subscription(), connections = /* @__PURE__ */ new Map();
      let currentValue;
      resources.add(() => {
        for (const entry of connections.values()) entry.subscription.unsubscribe();
        connections.clear();
      });
      const rewire = () => {
        ...
        for (let depth = 0; depth < parts.length && isObject(current); depth++) {
          ...
          if (!names) desired.set(target, names = /* @__PURE__ */ new Set());
          names.add(parts[depth]);
          names.add(parts.slice(depth).join("."));
        ...
        for (const [target, names] of desired) {
          ...
          const entry = { names, subscription: new import_rxjs.Subscription() };
          ...
          entry.subscription.add(propertyEvents(target).subscribe({
            ...
            error: (error) => observer.error(error),
            complete() {
              if (target === (proxyTargets.get(item) ?? item)) observer.complete();
            }
          }));
        }
      };
      try {
        currentValue = readProperty(item, property);
        rewire();
        if (notifyInitial && !observer.closed) observer.next({ sender: item, propertyName: property, value: currentValue, previous: void 0 });
      } catch (error) {
        observer.error(error);
      }
      return resources;
  ...
  return new import_rxjs.Observable((observer) => {
    const sub = propertyEvents(item).subscribe({
      next(event) {
        if (typeof property === "function" || property == null || allProperties(event.propertyName) || event.propertyName === property) {
          try {
            observer.next({ sender: item, propertyName: property ?? event.propertyName, value: readProperty(item, property), previous: event.previous });
          } catch (error) {
            observer.error(error);
          }
        }
    ...
    try {
      if (notifyInitial && !observer.closed) observer.next({ sender: item, propertyName: property, value: readProperty(item, property), previous: void 0 });
    } catch (error) {
      observer.error(error);
    }
    return sub;
...
function whenPropertyChanged(itemOrChanges, property, notifyInitial = true) {
  return (0, import_rxjs.isObservable)(itemOrChanges) ? itemOrChanges.pipe(mergeMany((item) => changedProperty(item, property, notifyInitial))) : changedProperty(itemOrChanges, property, notifyInitial);
}
function observeProperty(itemOrChanges, property, notifyInitial = true) {
  return whenPropertyChanged(itemOrChanges, property, notifyInitial).pipe((0, import_rxjs.map)((event) => event.value), (0, import_rxjs.distinctUntilChanged)());
}
const whenValueChanged = observeProperty;
function autoRefreshOnObservable(selector, options = {}) {
  ...
  return (source) => new import_rxjs.Observable((observer) => {
    ...
    const refresh = (entry) => {
      ...
      if (options.buffer > 0) {
        if (!timer) {
          timer = (options.scheduler ?? import_rxjs.asyncScheduler).schedule(() => {
            timer = null;
            flush();
          }, options.buffer);
          resources.add(timer);
    ...
    state = tracker({ add(entry) {
      entry.subscription.add((0, import_rxjs.from)(selector(entry.item, entry.key)).subscribe({ next: () => refresh(entry), error: (error) => observer.error(error) }));
    } });
...
function autoRefresh(property, options = {}) {
  ...
  return autoRefreshOnObservable((item) => {
    ...
    return options.throttle > 0 ? events.pipe((0, import_rxjs.debounceTime)(options.throttle, options.scheduler ?? import_rxjs.asyncScheduler)) : events;
  }, options);
}
  ...
  return (source) => new import_rxjs.Observable((observer) => {
    ...
    const start = (entry, previous) => {
      ...
      try {
        ...
        observable = (0, import_rxjs.isObservable)(selected) || selected?.then ? (0, import_rxjs.from)(selected) : (0, import_rxjs.of)(selected);
      } catch (error) {
        if (options.onError) {
      ...
      entry.subscription.add(observable.subscribe({ next(value) {
        if (entry.active) {
          entry.value = value;
          entry.hasValue = true;
          reconcile();
    ...
    state = tracker({
      ...
      refresh(entry) {
        if (options.transformOnRefresh) {
          entry.subscription.unsubscribe();
          entry.subscription = new import_rxjs.Subscription();
...
function transformAsync(factory, options = {}) {
  ...
  return (source) => new import_rxjs.Observable((observer) => {
    ...
    const drain = () => {
      ...
      draining = true;
      try {
        while (running < maximum && waiting.length) {
    ...
    const queued = (...arguments_) => new import_rxjs.Observable((destination) => {
      ...
      const job = { closed: false, start() {
        ...
        try {
          ...
          inner = ((0, import_rxjs.isObservable)(value) || value?.then ? (0, import_rxjs.from)(value) : (0, import_rxjs.of)(value)).subscribe({ next: (value2) => destination.next(value2), error(error) {
            finish();
...
function mergeMany(selector) {
  return (source) => new import_rxjs.Observable((observer) => {
    ...
    const state = tracker({ add(entry) {
      entry.subscription.add((0, import_rxjs.from)(selector(entry.item, entry.key)).subscribe({ next: (value) => observer.next(value), error: (error) => observer.error(error) }));
    } });
    resources.add(() => state.dispose());
    resources.add(source.subscribe({ next(changes) {
      try {
...
function mergeManyItems(selector) {
  return mergeMany((item, key) => (0, import_rxjs.from)(selector(item, key)).pipe((0, import_rxjs.map)((value) => ({ item, key, value }))));
}
...
function onItem(reason, action) {
  return (source) => new import_rxjs.Observable((observer) => source.subscribe({ next(changes) {
    try {
      for (const change of getChanges(changes)) {
        if (change.reason === reason || reason === "update" && change.reason === "replace") {
          if (reason === "update") action(change.current, change.previous, change.key);
          else action(change.current, change.key);
        } else if (reason === "add" && change.reason === "addRange" || reason === "remove" && (change.reason === "removeRange" || change.reason === "clear")) {
...
const onItemAdded = (action) => onItem("add", action);
const onItemUpdated = (action) => onItem("update", action);
const onItemRemoved = (action) => onItem("remove", action);
const onItemRefreshed = (action) => onItem("refresh", action);
function notEmpty() {
  return (0, import_rxjs.filter)((changes) => getChanges(changes).length > 0);
}
function skipInitial() {
  return (0, import_rxjs.skip)(1);
}
...
function watch(key) {
  return (source) => source.pipe((0, import_rxjs.map)((changes) => [...getChanges(changes)].filter((change) => Object.is(change.key, key))), (0, import_rxjs.filter)((changes) => changes.length > 0), mergeChangeRecords());
}
...
function batch(duration = 16, scheduler = import_rxjs.asyncScheduler) {
  if (!(duration > 0)) throw new RangeError("Batch duration must be positive");
  return (source) => source.pipe((0, import_rxjs.bufferTime)(duration, scheduler), (0, import_rxjs.filter)((sets) => sets.some((changes) => getChanges(changes).length)), (0, import_rxjs.map)((sets) => cs(sets.flatMap((changes) => [...getChanges(changes)]), sets[0]?.kind)));
}
  ...
  return (source) => new import_rxjs.Observable((observer) => {
    ...
    const armTimeout = () => {
      if (paused && options.timeout != null && !timeout) {
        timeout = (options.scheduler ?? import_rxjs.asyncScheduler).schedule(() => {
          paused = false;
          flush();
        }, options.timeout);
...
function toObservableChangeSet(keySelectorOrOptions, maybeOptions = {}) {
  ...
  return (source) => new import_rxjs.Observable((observer) => {
    const collection = keySelector ? new import_core.SourceCache(keySelector) : new import_core.SourceList();
    const resources = new import_rxjs.Subscription();
...
function createChangeSet(subscribe, keySelector) {
  return new import_rxjs.Observable((observer) => {
    const source = keySelector ? new import_core.SourceCache(keySelector) : new import_core.SourceList();
    const resources = new import_rxjs.Subscription();
...
function disposalOperator(disposer, completedAccessor) {
  return (source) => new import_rxjs.Observable((observer) => {
    const completion = new import_rxjs.ReplaySubject(1);
    const resources = new import_rxjs.Subscription();
...
function asyncDisposeMany(completedAccessor, disposer = (item) => item?.[Symbol.asyncDispose] ? item[Symbol.asyncDispose]() : item?.disposeAsync ? item.disposeAsync() : item?.DisposeAsync ? item.DisposeAsync() : release(item)) {
  return disposalOperator(disposer, completedAccessor);
}
const batchIf = bufferIf;
function bufferInitial(duration, scheduler = import_rxjs.asyncScheduler) {
...
function filterOnProperty(property, predicate, options = {}) {
  return (source) => source.pipe(autoRefresh(property, options), (0, import_operators.filter)(predicate));
}
const finallySafe = (action) => (0, import_rxjs.finalize)(action);
const ConnectionStatus = Object.freeze({ Pending: "pending", Loaded: "loaded", Errored: "errored", Completed: "completed" });
...
const watchValue = (key) => (source) => (typeof source.connect === "function" ? source.connect() : source).pipe(watch(key), (0, import_rxjs.map)((change) => change.current));
function whenAnyPropertyChanged(itemOrChanges, ...propertyNames) {
  const observe = (item) => propertyEvents(item).pipe((0, import_rxjs.filter)((event) => !propertyNames.length || allProperties(event.propertyName) || propertyNames.includes(event.propertyName)), (0, import_rxjs.map)(() => item));
  return (0, import_rxjs.isObservable)(itemOrChanges) ? itemOrChanges.pipe(mergeMany(observe)) : observe(itemOrChanges);
}
function whenChanged(item, properties, resultSelector) {
  const fields = Array.isArray(properties) ? properties : [properties];
  return (0, import_rxjs.combineLatest)(fields.map((property) => observeProperty(item, property))).pipe((0, import_rxjs.map)((values) => resultSelector ? resultSelector(item, ...values) : values.length === 1 ? values[0] : values));
}
function observeCollectionChanges(collection, eventName = "collectionchange") {
  const events = typeof collection.connect === "function" ? collection.connect() : (0, import_rxjs.isObservable)(collection) ? collection : (0, import_rxjs.fromEvent)(collection, eventName);
  return events.pipe((0, import_rxjs.map)((event) => ({ sender: collection, eventArgs: event?.detail ?? event })));
}
function trueFor(selector, condition, all) {
  return (source) => new import_rxjs.Observable((observer) => {
    ...
    state = tracker({ add(entry) {
      entry.subscription.add((0, import_rxjs.from)(selector(entry.item, entry.key)).subscribe({ next(value) {
        entry.value = value;
        entry.hasValue = true;
        evaluate();
      }, error: (error) => observer.error(error) }));
...
const trueForAll = (selector, condition = Boolean) => trueFor(selector, condition, true);
const trueForAny = (selector, condition = Boolean) => trueFor(selector, condition, false);
function transformManyAsync(factory, keySelector, options = {}) {
  return (source) => source.pipe(transformAsync(factory, options), (0, import_operators.transformMany)((items) => items, keySelector));
}
...
function refCount() {
  return (source) => {
    let session;
    return new import_rxjs.Observable((observer) => {
      if (!session) session = { subject: new import_rxjs.Subject(), references: 0, state: void 0, kind: "cache", upstream: new import_rxjs.Subscription() };
      const current = session;
      current.references++;
      const subscription = current.subject.subscribe(observer);
      if (current.state) {
        const initial = (0, import_core.snapshotChanges)(current.state, current.kind);
      ...
      if (current.references === 1 && !current.started) {
        ...
        current.upstream.add(source.subscribe({ next(changes) {
          ...
          (0, import_core.applyChanges)(current.state, changes);
...
function switchLatest() {
  return (sources) => new import_rxjs.Observable((observer) => {
    const resources = new import_rxjs.Subscription();
    ...
    resources.add(sources.subscribe({ next(source) {
      ...
      inner = (0, import_rxjs.from)(incoming).subscribe({ next(changes) {
