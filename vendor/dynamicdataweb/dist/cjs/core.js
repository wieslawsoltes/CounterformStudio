...
class Optional {
  ...
  static none() {
    return new Optional(void 0, false);
  }
  static of(value) {
  ...
  map(project) {
    return this.hasValue ? Optional.some(project(this._value)) : Optional.none();
  }
  ifHasValue(action) {
  ...
  equals(other) {
    return other instanceof Optional && other.hasValue === this.hasValue && (!this.hasValue || Object.is(this._value, other._value));
  }
  toString() {
...
Optional.ToOptional = Optional.of;
Optional.fromOptional = (optional) => optional.value;
...
const ChangeReason = reasons({ Add: "add", Update: "update", Remove: "remove", Refresh: "refresh", Moved: "move", Move: "move" });
const ListChangeReason = reasons({ Add: "add", AddRange: "addRange", Replace: "replace", Remove: "remove", RemoveRange: "removeRange", Refresh: "refresh", Moved: "move", Move: "move", Clear: "clear" });
const ChangeType = Object.freeze({ Item: "item", Range: "range" });
class Change {
...
function listFilter(seed, predicate) {
  ...
  const filteredIndex = (index) => {
    let count = 0;
    for (let i = 0; i < index; i++) count += +entries[i].pass;
    return count;
  };
  return { initial: () => snapshotChanges(entries.filter((e) => e.pass).map((e) => e.value), "list"), project(changes) {
    ...
    for (const change of changes) {
      ...
      switch (change.reason) {
        ...
        case "removeRange": {
          let at = rangeIndex(change);
          if (at >= 0) for (let j = 0; j < rangeItems(change).length; j++) remove(at);
        ...
        case "move": {
          const oldIndex = change.previousIndex, from = filteredIndex(oldIndex), [entry] = entries.splice(oldIndex, 1), to = filteredIndex(i);
          entries.splice(i, 0, entry);
          if (entry.pass && from !== to) output.push(new ListChange("move", entry.value, to, void 0, from));
          break;
        }
        case "update":
        case "replace": {
...
class SourceBase {
  constructor(kind) {
    ...
    this.countChanged = observableDecorator(new import_rxjs.Observable((observer) => {
      ...
      let last = this.count;
      const sub = this._counts.subscribe({ next: (count) => {
        if (count !== last) {
          last = count;
          observer.next(count);
        }
...
class SourceCache extends SourceBase {
...
class SourceList extends SourceBase {
  constructor(source) {
    super("list");
  ...
  removeRange(index, count) {
    return this._mutate(() => {
      this._index(index, true);
      if (!Number.isInteger(count) || count < 0 || index + count > this.count) throw new RangeError("Invalid removal range");
      if (!count) return;
      const values = this._data.splice(index, count);
  ...
  refreshMany(items) {
    return this._mutate(() => {
      ...
      for (let index = 0; index < this.count; index++) if (set.has(this._data[index])) this.refreshAt(index);
    });
  }
  load(items) {
    return this._mutate(() => {
...
class IntermediateCache extends SourceCache {
  constructor(source) {
...
class ChangeAwareCache extends SourceCache {
...
class ChangeAwareList extends SourceList {
  constructor(items, copyChanges = false) {
    super();
    this._captured = new ChangeSet([], "list");
    if (items instanceof ChangeAwareList) {
