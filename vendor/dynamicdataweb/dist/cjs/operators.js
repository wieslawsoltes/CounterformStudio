...
const observable = (value) => (0, import_rxjs.isObservable)(value) || !!value?.subscribe;
...
function changeSet(changes, kind, entries, metadata = {}) {
  const result = new import_core.ChangeSet(changes.map((change) => kind === "cache" ? new import_core.Change(change) : new import_core.ListChange(change)), kind, entries.map((e) => e.value));
...
function derived(setup) {
  return (source) => new import_rxjs.Observable((observer) => {
    const subscriptions = new import_rxjs.Subscription(), model = new Model();
    ...
    const emit = (entries, { kind = model.kind ?? "cache", refresh = model.refreshed, updated = /* @__PURE__ */ new Set(), suppressEmpty = true, metadata = {}, moves = true } = {}) => {
    ...
    const context = {
      ...
      emitRaw(changes, suppressEmpty = true, metadata = {}) {
        if (changes.length || !suppressEmpty) observer.next(Object.assign(new import_core.ChangeSet(changes.map((change) => new import_core.Change(change)), "cache"), metadata));
      },
      get loaded() {
        return loaded;
      },
...
const filterImmutable = (predicate, suppressEmptyChangeSets = true) => filter(predicate, { ignoreRefresh: true, suppressEmptyChangeSets });
const filterWithState = (state, predicate, suppressEmpty = true) => filter(state, predicate, suppressEmpty);
...
const transformImmutable = (factory) => transform(factory, { ignoreRefresh: true });
const convert = transform;
const cast = (converter = identity) => transform(converter);
const castToObject = () => transform(identity);
const ofType = (type) => {
...
const transformSafe = (factory, errorHandler, options = {}) => transform(factory, { ...options, errorHandler });
const transformWithInlineUpdate = (factory, updateAction, errorHandler, transformOnRefresh = false) => {
  ...
  return derived((ctx) => {
    ...
    const evaluateCache = (changes) => {
      ...
      if (comparerReady) {
        ...
        ctx.emitRaw(notifications, options.suppressEmptyChangeSets ?? true, metadata(ordered, initial ? "initialLoad" : "dataChanged"));
        delivered = true;
      }
    };
...
const page = (requests) => windowed("page", requests);
const virtualise = (requests) => windowed("virtual", requests);
const virtualize = virtualise;
const top = (comparerOrSize, requestedSize) => {
...
const reverse = () => derived((ctx) => () => ctx.emit(ctx.model.entries.slice().reverse()));
const sortAndPage = (comparer, requests, options) => (source) => source.pipe(sort(comparer, { ...options, waitForComparer: true, suppressEmptyChangeSets: false }), windowed("page", requests, { waitForRequest: true, operationOptions: options }));
const sortAndVirtualise = (comparer, requests, options) => (source) => source.pipe(sort(comparer, { ...options, waitForComparer: true, suppressEmptyChangeSets: false }), windowed("virtual", requests, { waitForRequest: true, operationOptions: options }));
const sortAndVirtualize = sortAndVirtualise;
const sortBy = (selector, direction = "ascending", options) => {
    ...
    return source.subscribe({ next: (changes) => {
      try {
        ...
        if (typeof target === "function") target(items, changes);
          ...
          clear.call(collection);
          add.call(collection, items);
        });
...
const bindToObservableList = bind;
const bindToObservableCollection = bind;
const sortAndBind = (targetOrComparer, comparerOrTarget, options) => {
...
const asObservableCache = (sourceOrKeySelector, keySelector) => observable(sourceOrKeySelector) ? new import_core.ObservableCache(sourceOrKeySelector, keySelector) : (source) => new import_core.ObservableCache(source, sourceOrKeySelector);
const asObservableList = (source) => observable(source) ? new import_core.ObservableList(source) : (input) => new import_core.ObservableList(input);
function changeKey(selector) {
  if (typeof selector !== "function") throw new TypeError("changeKey requires a selector.");
  return derived((ctx) => {
    const keyed = /* @__PURE__ */ new Map();
    return () => {
      const next = /* @__PURE__ */ new Map(), live = /* @__PURE__ */ new Set(), refresh = /* @__PURE__ */ new Set();
      for (const e of ctx.model.entries) {
        live.add(e.id);
...
const removeKey = () => derived((ctx) => () => ctx.emit(ctx.model.entries, { kind: "list" }));
