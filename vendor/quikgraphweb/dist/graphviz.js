...
class GraphvizAlgorithm {
  constructor(graph, imageType = GraphvizImageType.Png) {
    ...
    this.FormatVertex = new EventHook();
    this.FormatEdge = new EventHook();
    this.FormatCluster = new EventHook();
    this.Output = null;
    this.ClusterCount = 0;
  }
  get VisitedGraph() {
  ...
  Generate(engine, outputFilePath) {
    ...
    this.ClusterCount = 0;
    const ids = new Map(Array.from(this.VisitedGraph.Vertices, (v, i) => [v, i]));
    const remainingVertices = new Set(ids.keys());
    const remainingEdges = new Set(this.VisitedGraph.Edges);
