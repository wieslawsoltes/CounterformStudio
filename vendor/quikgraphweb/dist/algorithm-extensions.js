...
function GetVertexIdentity(graph) {
  ...
  const types = new Set([...graph.Vertices].map((v) => typeof v));
...
function components(graph, Type, output = new Map()) {
...
function ConnectedComponents(graph, output = new Map()) {
  return components(graph, Structural.ConnectedComponentsAlgorithm, output);
}
function StronglyConnectedComponents(graph, output = new Map()) {
  return components(graph, Structural.StronglyConnectedComponentsAlgorithm, output);
}
function WeaklyConnectedComponents(graph, output = new Map()) {
...
function OddVertices(graph) {
  required(graph, "graph");
  const counts = new Map([...graph.Vertices].map((v) => [v, 0]));
  for (const e of graph.Edges) {
...
function MaximumFlow(graph, capacities, source, sink, edgeFactory = (s, t) => new Core.Edge(s, t), augmentor) {
  ...
  required(sink, "sink");
  if (same(source, sink)) throw new Error("Source and sink must differ.");
