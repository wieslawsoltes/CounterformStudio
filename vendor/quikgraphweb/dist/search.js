...
function color(map, vertex) {
  if (!map.has(vertex)) throw algorithmError("VertexNotFoundException", "Vertex color not available.");
...
class BreadthFirstSearchAlgorithm extends RootedAlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parseSearch(input);
    super(host, graph);
    this.VertexQueue = args[0] ?? null;
    this.VerticesColors = args[1] ?? new Map();
...
class DepthFirstSearchAlgorithm extends RootedAlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parseSearch(input);
    super(host, graph);
  ...
  set MaxDepth(value) {
    if (!Number.isInteger(value) || value < 0) throw algorithmError("ArgumentOutOfRangeException", "MaxDepth must be a non-negative integer.");
...
class EdgeDepthFirstSearchAlgorithm extends RootedAlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parseSearch(input);
    super(host, graph);
    this.EdgesColors = args.length ? requireValue(args[0], "edgesColors") : new Map();
    this.ProcessAllComponents = false;
    this._maxDepth = 2147483647;
...
class BestFirstFrontierSearchAlgorithm extends RootedSearchAlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parseSearch(input);
    super(host, graph);
  ...
  InternalCompute() {
    ...
    while (heap.Count) {
      ...
      for (const edge of this.VisitedGraph.OutEdges(vertex)) {
        ...
        if (weight < 0) throw algorithmError("NegativeWeightException", "Best-first search requires non-negative weights.");
