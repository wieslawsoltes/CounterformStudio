...
var xml_validation_exports = {};
__export(xml_validation_exports, {
  CreateGraphMLSchemaValidator: () => CreateGraphMLSchemaValidator,
  DeserializeAndValidateGraphML: () => DeserializeAndValidateGraphML,
  GraphMLSchemaValidator: () => GraphMLSchemaValidator,
  GraphMLValidationError: () => GraphMLValidationError,
  ValidateGraphMLSchema: () => ValidateGraphMLSchema
});
module.exports = __toCommonJS(xml_validation_exports);
var import_serialization = require("./serialization.js"), schemaBase = "quikgraphweb://schemas/graphml/1.1/", schemaNames = ["graphml.xsd", "graphml-structure.xsd", "graphml-attributes.xsd", "graphml-parseinfo.xsd", "xlink.xsd"], providers = /* @__PURE__ */ new WeakMap(), encoder = new TextEncoder(), decoder = new TextDecoder("utf-8", { fatal: !0 }), constructionKey = /* @__PURE__ */ Symbol("GraphMLSchemaValidator");
...
function schemaText(name) {
  return decoder.decode(import_serialization.GraphMLResourceResolver.GetResource(name)).replace(/schemaLocation="([^"]+)"/g, (_, location) => {
...
`), cause ? { cause } : void 0), this.name = "GraphMLValidationError", this.Errors = errors;
  ...
  Deserialize(input, options = {}) {
    ...
    return this.ValidateAndThrow(text, { filename: options.filename }), new import_serialization.GraphMLDeserializer(options).Deserialize(text, options.graph, options.vertexFactory, options.edgeFactory);
  }
  /** Serialize and validate before invoking an optional output writer. */
  Serialize(graph, options = {}) {
    this._ensureActive();
    let text = new import_serialization.GraphMLSerializer(options).Serialize(graph, options);
