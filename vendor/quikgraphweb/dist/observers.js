...
class VertexPredecessorRecorderObserver {
  constructor(verticesPredecessors = new Map()) {
    this.VerticesPredecessors = requireValue(verticesPredecessors, "verticesPredecessors");
  }
  Attach(algorithm) {
    return attach(algorithm, { TreeEdge: (edge) => this.VerticesPredecessors.set(edge.Target, edge) });
  }
  TryGetPath(vertex) {
    return predecessorPath(this.VerticesPredecessors, vertex);
  }
}
class UndirectedVertexPredecessorRecorderObserver extends VertexPredecessorRecorderObserver {
  ...
  TryGetPath(vertex) {
    return predecessorPath(this.VerticesPredecessors, vertex, true);
  }
}
class VertexDistanceRecorderObserver {
  constructor(edgeWeights, distanceRelaxer = DistanceRelaxers.EdgeShortestDistance, distances = new Map()) {
    this.EdgeWeights = requireValue(edgeWeights, "edgeWeights");
    this.DistanceRelaxer = requireValue(distanceRelaxer, "distanceRelaxer");
    this.Distances = requireValue(distances, "distances");
  }
...
class VertexTimeStamperObserver {
  constructor(discoverTimes, finishTimes) {
    ...
    this.FinishTimes = arguments.length === 1 ? null : arguments.length > 1 ? requireValue(finishTimes, "finishTimes") : new Map();
    this._currentTime = 0;
  }
  Attach(algorithm) {
...
class VertexRecorderObserver {
  constructor(vertices = []) {
    this.Vertices = [...requireValue(vertices, "vertices")];
  }
  Attach(algorithm) {
...
class VertexPredecessorPathRecorderObserver extends VertexPredecessorRecorderObserver {
  ...
  Attach(algorithm) {
    return attach(algorithm, {
      ...
      FinishVertex: (vertex) => {
        for (const edge of this.VerticesPredecessors.values()) if (sameVertex(edge.Source, vertex)) return;
...
class EdgePredecessorRecorderObserver {
  constructor(edgesPredecessors = new Map()) {
    this.EdgesPredecessors = requireValue(edgesPredecessors, "edgesPredecessors");
    this.EndPathEdges = [];
  }
  Attach(algorithm) {
    return attach(algorithm, {
      DiscoverTreeEdge: (edge, targetEdge) => {
        if (!valueEquals(edge, targetEdge)) this.EdgesPredecessors.set(targetEdge, edge);
      },
      FinishEdge: (edge) => {
        for (const predecessor of this.EdgesPredecessors.values()) if (valueEquals(predecessor, edge)) return;
        this.EndPathEdges.push(edge);
      }
    });
  }
  Path(startingEdge) {
    requireValue(startingEdge, "startingEdge");
    const path = [], seen = new Set();
    let edge = startingEdge;
    while (edge !== void 0) {
      if (seen.has(edge)) throw algorithmError("InvalidOperationException", "The edge predecessor map contains a cycle.");
      seen.add(edge);
      path.push(edge);
  ...
  *AllPaths() {
  ...
  MergedPath(startingEdge, colors) {
    requireValue(startingEdge, "startingEdge");
    requireValue(colors, "colors");
    const path = [];
    let edge = startingEdge;
    while (edge !== void 0) {
      if (!colors.has(edge)) throw algorithmError("KeyNotFoundException", "No color recorded for edge.");
      if (colors.get(edge) !== GraphColor.White) break;
      colors.set(edge, GraphColor.Black);
      path.push(edge);
      edge = this.EdgesPredecessors.get(edge);
    }
    return path.reverse();
  ...
  *AllMergedPaths() {
    ...
    for (const [edge, parent] of this.EdgesPredecessors) {
      colors.set(edge, GraphColor.White);
      colors.set(parent, GraphColor.White);
    }
    for (const edge of this.EndPathEdges) {
      if (!colors.has(edge)) colors.set(edge, GraphColor.White);
