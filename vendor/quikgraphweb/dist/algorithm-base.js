...
import { requireValue as requireValue2 } from "./core.js";
import { GraphColor } from "./core.js";
...
function events(owner, names) {
  for (const name of names.split(" ")) if (!owner[name]) owner[name] = new EventHook();
}
class CancelManager {
  constructor() {
    this.IsCancelling = false;
    this.CancelRequested = new EventHook();
    this.CancelReset = new EventHook();
...
class AlgorithmBase {
  constructor(host, graph) {
    ...
    this.VisitedGraph = requireValue(graph, "visitedGraph");
