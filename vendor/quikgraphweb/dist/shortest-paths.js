...
function parse(input) {
  ...
  const graph = requireValue(args.shift(), "visitedGraph");
  if (typeof graph.ContainsVertex !== "function") throw algorithmError("ArgumentException", "visitedGraph must implement ContainsVertex.");
  return { host, graph, args };
}
function checkedWeight(weights, edge, nonnegative = false) {
  const weight = weights(edge);
  if (typeof weight !== "number" || !Number.isFinite(weight)) throw algorithmError("ArgumentException", "Edge weights must be finite numbers.");
  if (nonnegative && weight < 0) throw algorithmError("NegativeWeightException", "Algorithm requires non-negative edge weights.");
  return weight;
}
function predecessorPath(predecessors, vertex, undirected = false) {
  ...
  while (predecessors.has(vertex)) {
    if (seen.has(vertex)) throw algorithmError("InvalidOperationException", "The predecessor map contains a cycle.");
    seen.add(vertex);
    const edge = predecessors.get(vertex);
    path.push(edge);
    vertex = undirected && sameVertex(edge.Source, vertex) ? edge.Target : edge.Source;
  }
  return path.length ? path.reverse() : void 0;
}
class ShortestPathAlgorithmBase extends RootedAlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parse(input);
    super(host, graph);
    this.Weights = requireValue(args[0], "edgeWeights");
    this.DistanceRelaxer = args.length > 1 ? requireValue(args[1], "distanceRelaxer") : DistanceRelaxers.ShortestDistance;
    this.Distances = null;
  ...
  TryGetDistance(v) {
    requireValue(v, "vertex");
    if (!this.Distances) throw algorithmError("InvalidOperationException", "Run the algorithm before.");
    return this.Distances.get(v);
  }
  GetDistance(v) {
    const result = this.TryGetDistance(v);
    if (result === void 0) throw algorithmError("VertexNotFoundException", "Vertex distance not available.");
    return result;
  }
  GetDistances() {
  ...
  OnTreeEdge(edge, reversed = false) {
    if (this._undirected) this.TreeEdge.emit(this, new UndirectedEdgeEventArgs(edge, reversed));
  ...
  GetVertexColor(v) {
    if (!this.VerticesColors.has(v)) throw algorithmError("VertexNotFoundException", "Vertex color not available.");
  ...
  _emitEdge(name, edge, source) {
    if (this._undirected && name !== "ExamineEdge") this[name].emit(this, new UndirectedEdgeEventArgs(edge, sameVertex(edge.Target, source)));
...
class AStarShortestPathAlgorithm extends DijkstraShortestPathAlgorithm {
  constructor(...input) {
    const { host, graph, args } = parse(input);
    super(host, graph, args[0], args.length > 2 ? requireValue(args[2], "distanceRelaxer") : DistanceRelaxers.ShortestDistance);
    this.CostHeuristic = requireValue(args[1], "costHeuristic");
  }
}
class BellmanFordShortestPathAlgorithm extends ShortestPathAlgorithmBase {
  constructor(...args) {
    ...
    events(this, "EdgeMinimized EdgeNotMinimized");
  }
  Initialize() {
    super.Initialize();
    this.FoundNegativeCycle = false;
    ...
    if (root === void 0) throw algorithmError("InvalidOperationException", "Graph is empty.");
    this.Distances.set(root, 0);
  }
  InternalCompute() {
...
class FloydWarshallAllShortestPathAlgorithm extends AlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parse(input);
    super(host, graph);
    this.Weights = requireValue(args[0], "edgeWeights");
  ...
  Initialize() {
    ...
    this._next.clear();
    for (const v of this.VisitedGraph.Vertices) {
      this.Distances.set(v, new Map([[v, 0]]));
      this._next.set(v, new Map());
    }
    for (const edge of this.VisitedGraph.Edges) {
  ...
  TryGetPath(source, target) {
    requireValue(source, "source");
    requireValue(target, "target");
    if (sameVertex(source, target) || !this._next.get(source)?.has(target)) return void 0;
    const path = [], seen = new Set();
    while (!sameVertex(source, target)) {
      if (seen.has(source)) throw algorithmError("InvalidOperationException", "Cycle in shortest path.");
      seen.add(source);
      const edge = this._next.get(source)?.get(target);
      if (!edge) return void 0;
      path.push(edge);
      source = !this.VisitedGraph.IsDirected && sameVertex(edge.Target, source) ? edge.Source : edge.Target;
    }
    return path;
  }
  Dump(writer) {
    requireValue(writer, "writer");
    const lines = ["data:"];
...
class SortedPath {
  constructor(edges) {
    this.Edges = [...requireValue(edges, "edges")];
  ...
  Equals(other) {
    return other instanceof SortedPath && this.Count === other.Count && this.Edges.every((edge, i) => valueEquals(edge, other.Edges[i]));
  }
  GetHashCode() {
...
function shortestAvoiding(graph, source, target, weights, bannedVertices = new Set(), bannedEdges = new Set()) {
  const distances = new Map([[source, 0]]), predecessors = new Map(), heap = new AlgorithmHeap();
...
class YenShortestPathsAlgorithm {
  ...
  constructor(graph, source, target, k, edgeWeights = null, filter = null) {
    this.VisitedGraph = requireValue(graph, "graph").Clone();
    requireValue(source, "source");
    requireValue(target, "target");
  ...
  Execute() {
    ...
    if (first === void 0 || !first.length) throw algorithmError("NoPathFoundException", "No path found between the supplied vertices.");
...
class RankedShortestPathAlgorithmBase extends RootedSearchAlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parse(input);
    super(host, graph);
    this.DistanceRelaxer = args[0] ?? DistanceRelaxers.ShortestDistance;
  ...
  set ShortestPathCount(value) {
    if (!Number.isInteger(value) || value <= 1) throw algorithmError("ArgumentOutOfRangeException", "ShortestPathCount must be more than 1.");
...
class HoffmanPavleyRankedShortestPathAlgorithm extends RankedShortestPathAlgorithmBase {
  constructor(...input) {
    const { host, graph, args } = parse(input);
    super(host, graph, args.length > 1 ? requireValue(args[1], "distanceRelaxer") : DistanceRelaxers.ShortestDistance);
    this.Weights = requireValue(args[0], "edgeWeights");
  }
  InternalCompute() {
    const root = this.GetAndAssertRootInGraph(), target = this.TryGetTargetVertex();
    if (target === void 0) throw algorithmError("InvalidOperationException", "Target vertex not set.");
    this.AssertRootInGraph(target);
    if (sameVertex(root, target)) return;
    const graph = this.VisitedGraph, incoming = new Map([...graph.Vertices].map((v) => [v, []]));
    for (const edge of graph.Edges) incoming.get(edge.Target).push(edge);
    const distances = new Map([[target, 0]]), successors = new Map(), treeQueue = new AlgorithmHeap((a, b) => this.DistanceRelaxer.Compare(a.priority, b.priority));
    ...
    const append = (path, vertex) => {
      ...
      return sameVertex(vertex, target);
    };
    const first = [];
