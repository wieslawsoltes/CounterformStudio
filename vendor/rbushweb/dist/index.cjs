...
__export(index_exports, {
  ...
  Node: () => Node,
  RBush: () => RBush,
  RBushExtensions: () => RBushExtensions,
  default: () => index_default,
  distanceTo: () => DistanceTo,
  knn: () => Knn
});
module.exports = __toCommonJS(index_exports);
var minX = (b) => b.MinX === void 0 ? b.minX : b.MinX;
var minY = (b) => b.MinY === void 0 ? b.minY : b.MinY;
var maxX = (b) => b.MaxX === void 0 ? b.maxX : b.MaxX;
var maxY = (b) => b.MaxY === void 0 ? b.maxY : b.MaxY;
var area = (b) => Math.max(maxX(b) - minX(b), 0) * Math.max(maxY(b) - minY(b), 0);
var margin = (b) => Math.max(maxX(b) - minX(b), 0) + Math.max(maxY(b) - minY(b), 0);
var intersects = (a, b) => minX(a) <= maxX(b) && minY(a) <= maxY(b) && maxX(a) >= minX(b) && maxY(a) >= minY(b);
var contains = (a, b) => minX(a) <= minX(b) && minY(a) <= minY(b) && maxX(a) >= maxX(b) && maxY(a) >= maxY(b);
var numeric = (n) => typeof n === "number" && !Number.isNaN(n);
var inverted = (b) => minX(b) > maxX(b) || minY(b) > maxY(b);
function boxOf(item) {
...
function extendedArea(a, b) {
  return Math.max(Math.max(maxX(a), maxX(b)) - Math.min(minX(a), minX(b)), 0) * Math.max(Math.max(maxY(a), maxY(b)) - Math.min(minY(a), minY(b)), 0);
}
var Envelope = class _Envelope {
  ...
  Extend(other) {
    return extend(this, boxOf(other));
  }
  Intersection(other) {
    const b = boxOf(other);
    return new _Envelope(Math.max(this.MinX, minX(b)), Math.max(this.MinY, minY(b)), Math.min(this.MaxX, maxX(b)), Math.min(this.MaxY, maxY(b)));
  }
  Contains(other) {
    return contains(this, boxOf(other));
  }
  Intersects(other) {
    return intersects(this, boxOf(other));
  }
  DistanceTo(x, y) {
    return DistanceTo(this, x, y);
  }
  Equals(other) {
...
function DistanceTo(envelope, x, y) {
...
var internalNode = /* @__PURE__ */ Symbol("internal R-tree node");
var readonlyArray = {
  set() {
    throw new TypeError("Node.Children is read-only.");
  },
  deleteProperty() {
    throw new TypeError("Node.Children is read-only.");
  },
...
var Node = class {
  ...
  get Height() {
    return nodes.get(this).height;
  }
  get IsLeaf() {
    return this.Height === 1;
  }
  get Envelope() {
    return nodes.get(this).envelope;
  }
  get children() {
...
function enclosing(items, getBox) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    x1 = Math.min(x1, minX(b));
    y1 = Math.min(y1, minY(b));
    x2 = Math.max(x2, maxX(b));
    y2 = Math.max(y2, maxY(b));
  }
...
var trees = /* @__PURE__ */ new WeakMap();
var RBush = class _RBush {
  ...
  get Root() {
    return trees.get(this).root;
  }
  get Envelope() {
    return this.Root.Envelope;
  }
  get Count() {
    return trees.get(this).count;
  }
  get MaxEntries() {
    return trees.get(this).maxEntries;
  }
  Clear() {
  ...
  Search(boundingBox) {
    const state = trees.get(this);
    const result = [];
    const query = boundingBox === void 0 ? null : checkBox(boxOf(boundingBox));
  ...
  Knn(k, x, y, maxDistance, predicate) {
    return Knn(this, k, x, y, maxDistance, predicate);
  }
  ...
  GetStats() {
    ...
    while (stack.length) {
      ...
      if (node.IsLeaf) {
        leafCount++;
        if (!nodes.get(node).items.length) emptyLeafCount++;
  ...
  Validate() {
    ...
    while (stack.length) {
      ...
      for (const item of items) {
        ...
        if (node.IsLeaf) {
          count++;
          if (inverted(state.getBox(item))) invertedCount++;
        } else {
  ...
  FromJSON(snapshot, deserializeItem = (item) => item) {
    ...
    const state = trees.get(this), seen = /* @__PURE__ */ new WeakSet();
    let count = 0, invertedCount = 0;
    const decode = (value, expectedHeight) => {
      if (!value || typeof value !== "object" || seen.has(value) || !Number.isInteger(value.height) || value.height < 1 || value.height > 64 || expectedHeight !== void 0 && value.height !== expectedHeight || !Array.isArray(value.children) || value.children.length > snapshot.maxEntries) throw new TypeError("Invalid or cyclic RBushWeb node.");
    ...
    state.count = count;
    state.invertedCount = invertedCount;
    state.maxEntries = snapshot.maxEntries;
    state.minEntries = Math.max(2, Math.ceil(snapshot.maxEntries * 0.4));
    return this;
  }
  static FromJSON(snapshot, deserializeItem, options) {
    return new _RBush(options).FromJSON(snapshot, deserializeItem);
  }
  get root() {
    return this.Root;
  }
  ...
  search(boundingBox) {
    return this.Search(boundingBox);
  }
  all() {
    return this.Search();
  }
  ...
  knn(k, x, y, maxDistance, predicate) {
    return this.Knn(k, x, y, maxDistance, predicate);
  }
  collides(boundingBox) {
    return this.Collides(boundingBox);
  }
  getStats() {
  ...
  toJSON(serializeItem) {
    return this.ToJSON(typeof serializeItem === "function" ? serializeItem : void 0);
  }
  fromJSON(snapshot, deserializeItem) {
    return this.FromJSON(snapshot, deserializeItem);
  }
  static fromJSON(snapshot, deserializeItem, options) {
    return _RBush.FromJSON(snapshot, deserializeItem, options);
  }
  [Symbol.iterator]() {
...
function splitNode(state, node) {
  ...
  for (let i = 1; i < items.length; i++) {
    ...
    const overlap = Math.max(Math.min(left.MaxX, right.MaxX) - Math.max(left.MinX, right.MinX), 0) * Math.max(Math.min(left.MaxY, right.MaxY) - Math.max(left.MinY, right.MinY), 0);
...
function remove(state, node, item, box) {
  ...
  if (node.IsLeaf) {
    ...
    const removed2 = items.length - write;
    if (!removed2) return false;
    items.length = write;
    state.count -= removed2;
    reset(node);
    return true;
  }
  let removed = false;
...
function Knn(tree, k, x, y, maxDistance, predicate) {
  ...
  if (!state || state.invertedCount || !Number.isFinite(x) || !Number.isFinite(y)) {
    ...
    for (const candidate of candidates) {
      ...
      if (!predicate || predicate(candidate.item)) result2.push(candidate.item);
      if (k > 0 && result2.length === k) break;
    }
    return result2;
  }
...
var RBushExtensions = Object.freeze({ Knn, DistanceTo, knn: Knn, distanceTo: DistanceTo });
var index_default = RBush;
