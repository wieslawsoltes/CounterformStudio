...
for (const name of Object.getOwnPropertyNames(baseProbe))
    reservedMembers.add(name);
baseProbe.Dispose();
for (const name of Object.getOwnPropertyNames(ReactiveObjectBase.prototype))
    reservedMembers.add(name);
...
function validInitial(initial, type, nullable, label) {
    ...
    if (!matches || (typeof initial === 'number' && !Number.isFinite(initial)))
        throw new TypeError(`${label} does not match its declared type '${type}'.`);
}
/** Validates names, imported factories, types, validation rules, and collisions. */
export function ValidateGenerationSchema(value) {
    record(value, 'Schema');
    rejectUnknown(value, ['className', 'imports', 'properties', 'computed', 'commands'], 'schema');
    identifier(value.className, 'className');
    if (reservedImports.has(value.className))
        throw new TypeError('className conflicts with a runtime import.');
    ...
    for (const [name, moduleName] of Object.entries(imports)) {
        ...
        if (reservedImports.has(name) || name === value.className)
            throw new TypeError(`Imported name '${name}' conflicts with a generated declaration.`);
        if (typeof moduleName !== 'string' || !moduleName.trim() || /[\r\n\0]/.test(moduleName))
            throw new TypeError(`Invalid module for '${name}'.`);
    }
    const allNames = new Set();
    const requireFactory = (factory, label) => {
        identifier(factory, label);
        if (!Object.hasOwn(imports, factory))
            throw new TypeError(`${label} '${factory}' is missing from imports.`);
    };
    for (const section of ['properties', 'computed', 'commands']) {
        ...
        for (const [name, raw] of Object.entries(members)) {
            ...
            if (allNames.has(name) || reservedMembers.has(name))
                throw new TypeError(`Duplicate or reserved member '${name}'.`);
            allNames.add(name);
            record(raw, `${section}.${name}`);
            if (section === 'properties') {
                rejectUnknown(raw, ['initial', 'type', 'nullable', 'validate', 'dependents'], `property '${name}'`);
                if (!Object.hasOwn(raw, 'initial'))
                    throw new TypeError(`Property '${name}' requires initial.`);
                ...
                if (raw.dependents !== undefined) {
                    if (!Array.isArray(raw.dependents))
                        throw new TypeError(`${name}.dependents must be an array.`);
                    for (const dependent of raw.dependents)
                        identifier(dependent, 'Dependent name');
                }
                if (raw.validate !== undefined) {
                    ...
                    if (raw.validate.minimum !== undefined && raw.validate.maximum !== undefined && Number(raw.validate.minimum) > Number(raw.validate.maximum))
                        throw new TypeError(`${name}: minimum exceeds maximum.`);
                    if (raw.validate.minLength !== undefined && raw.validate.maxLength !== undefined && Number(raw.validate.minLength) > Number(raw.validate.maxLength))
                        throw new TypeError(`${name}: minLength exceeds maxLength.`);
                    if (raw.validate.pattern !== undefined) {
                        if (typeof raw.validate.pattern !== 'string')
                            throw new TypeError(`${name}.validate.pattern must be a regular-expression string.`);
                ...
                if (Object.hasOwn(raw, 'initialValue'))
                    validInitial(raw.initialValue, raw.type, raw.nullable, name);
            }
            else {
                rejectUnknown(raw, ['execute', 'kind', 'canExecute', 'inputType', 'outputType'], `command '${name}'`);
                requireFactory(raw.execute, `${name}.execute`);
                if (raw.canExecute !== undefined)
                    requireFactory(raw.canExecute, `${name}.canExecute`);
                if (raw.kind !== undefined && !['sync', 'task', 'observable'].includes(String(raw.kind)))
                    throw new TypeError(`${name}.kind must be sync, task, or observable.`);
...
function literal(value) {
    ...
    if (serialized === undefined)
        throw new TypeError('Initial values must be JSON serializable.');
...
function validationCondition(_name, property, variable = 'value') {
    ...
    if (validation?.required)
        rules.push(`(${variable} !== null && ${variable} !== undefined && ${variable} !== "")`);
    if (validation?.minimum !== undefined)
        rules.push(`(typeof ${variable} === "number" && ${variable} >= ${validation.minimum})`);
    if (validation?.maximum !== undefined)
        rules.push(`(typeof ${variable} === "number" && ${variable} <= ${validation.maximum})`);
    if (validation?.minLength !== undefined)
        rules.push(`((typeof ${variable} === "string" || Array.isArray(${variable})) && ${variable}.length >= ${validation.minLength})`);
    if (validation?.maxLength !== undefined)
        rules.push(`((typeof ${variable} === "string" || Array.isArray(${variable})) && ${variable}.length <= ${validation.maxLength})`);
    if (validation?.pattern !== undefined)
        rules.push(`(typeof ${variable} === "string" && new RegExp(${literal(validation.pattern)}).test(${variable}))`);
    return rules;
}
/** Generates reviewable ESM source, without eval or embedding executable expressions from JSON. */
export function GenerateViewModelSource(schema, options = {}) {
    ValidateGenerationSchema(schema);
    if (options.language !== undefined && options.language !== 'ts' && options.language !== 'js')
        throw new TypeError('language must be ts or js.');
    const ts = options.language !== 'js';
    const moduleName = options.moduleName ?? '@wieslawsoltes/reactiveweb';
    if (!moduleName || /[\r\n\0]/.test(moduleName))
        throw new TypeError('moduleName must be a valid module specifier.');
    ...
    for (const [source, names] of groupedImports)
        lines.push(`import { ${names.join(', ')} } from ${literal(source)};`);
    ...
    if (ts) {
        for (const [name, definition] of computed)
            lines.push(`  declare readonly ${name}: ${definition.type ?? inferredType(definition.initialValue)}${definition.nullable ? ' | null' : ''}${definition.initialValue === undefined ? ' | undefined' : ''};`);
        for (const [name, definition] of commands)
            lines.push(`  readonly ${name}: ReactiveCommand<${definition.inputType ?? 'unknown'}, ${definition.outputType ?? 'unknown'}>;`);
    }
    ...
    for (const [name, definition] of properties)
        lines.push(`      this.${name} = Object.hasOwn(initial, ${literal(name)}) ? initial.${name}${ts ? '!' : ''} : ${literal(definition.initial)};`);
    for (const [name, definition] of computed) {
...
export async function RunGenerator(args, io = { stdout: text => process.stdout.write(text), stderr: text => process.stderr.write(text) }) {
    try {
        ...
        for (let index = 0; index < args.length; index++) {
            ...
            if (['--out', '--language', '--module'].includes(arg)) {
                if (seen.has(arg))
                    throw new TypeError(`Duplicate option ${arg}.`);
                seen.add(arg);
                ...
                throw new TypeError('Only one schema input is accepted.');
            else
                input = arg;
        }
        if (!input || !output)
            throw new TypeError(usage.trim());
        if (resolve(input) === resolve(output))
            throw new TypeError('Output must not overwrite the input schema.');
        if (!language)
            language = ['.js', '.mjs'].includes(extname(output)) ? 'js' : 'ts';
        ...
        try {
            await writeFile(output, generated, { encoding: 'utf8', flag: force ? 'w' : 'wx' });
        }
        catch (error) {
...
function isMainModule() {
    try {
        return !!process.argv[1] && import.meta.url === pathToFileURL(realpathSync(resolve(process.argv[1]))).href;
    }
    catch {
        return false;
    }
