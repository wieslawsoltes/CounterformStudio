...
import { toPDF, PDFEditor } from "./formats-pdf.js";
import { toDOCX as toDOCX2, fromDOCX as fromDOCX2 } from "./formats-docx.js";
import { toPDF as toPDF2 } from "./formats-pdf.js";
let nextId = 0;
...
const docFromNodes = (blocks, props = {}) => FlowDocument.FromJSON(
  ...
  for (const [key, value] of Object.entries(floatingProperties(props))) {
    try {
      if ((key === "Width" || key === "Height") && value && typeof value === "object") {
        if (kind !== "Figure" || Array.isArray(value) || typeof value.Value !== "number" || !Object.values(FigureUnitType).includes(value.FigureUnitType))
          continue;
        ...
        const allowed = key === "HorizontalAnchor" ? FigureHorizontalAnchor : key === "VerticalAnchor" ? FigureVerticalAnchor : WrapDirection;
...
function htmlNode(node, phrasingBlocks = false) {
  const p = node.props ?? {}, css = styles(p), attr = css ? ` style="${escapeMarkup(css)}"` : "";
  ...
  const wrap = (tag, extra = "") => phrasingBlocks && floatingBlockDisplays[tag] ? `<span data-rt-block="${tag}" style="display:${floatingBlockDisplays[tag]}${css ? ";" + escapeMarkup(css) : ""}"${extra}>${body}</span>` : `<${tag}${attr}${extra}>${body}</${tag}>`;
  switch (node.type) {
    case "FlowDocument":
    ...
    case "Run": {
      ...
      return runCss ? `<span style="${escapeMarkup(runCss)}">${escapeMarkup(text)}</span>` : escapeMarkup(text);
    }
    case "Bold":
      return wrap("strong");
    case "Italic":
    ...
    case "Hyperlink": {
      ...
      return href ? wrap("a", ` href="${escapeMarkup(href)}" rel="noopener noreferrer"`) : wrap("span");
    }
    case "LineBreak":
      return "<br>";
    case "List": {
    ...
    case "Equation": {
      const options = equationOptions(node), rendered = renderEquation(options);
    ...
    case "Image": {
      ...
      return src ? `<img${imageCss ? ` style="${escapeMarkup(imageCss)}"` : ""}${Object.keys(layout).length ? ` data-rt-layout="${escapeMarkup(JSON.stringify(layout))}"` : ""} src="${escapeMarkup(src)}" alt="${escapeMarkup(p.AlternativeText ?? "")}">` : escapeMarkup(p.AlternativeText ?? "");
    }
    case "Figure":
    case "Floater":
      return `<span data-rt-floating="${node.type}" data-rt-layout="${escapeMarkup(JSON.stringify(safeFloatingProperties(p, node.type)))}" style="${escapeMarkup(css + ";" + floatingStyles(p))}">${body}</span>`;
...
function htmlInlines(nodes, preserveSpace = false) {
  ...
  for (const node of nodes) {
    ...
    if (node.attrs["data-rt-equation"] || name === "math") {
      ...
      const source = name === "math" ? sanitizeMathML(serializeMathNode(node)) : node.attrs["data-rt-source"] ?? "";
      renderEquation({
        Source: source,
        Format: format,
...
const equationMarkup = (source, display) => `<span data-rt-equation="latex" data-rt-source="${escapeMarkup(source)}" data-rt-display="${display}">[equation]</span>`;
markdownParser.use({
  extensions: [
    {
      name: "blockEquation",
...
function fromXAML(xaml) {
  ...
  const convert = (node, parent = "", inheritedPreserve = false) => {
    ...
    const text = name === "Run" ? node.attrs.Text ?? textContent(node) : void 0;
    return [
      makeNode(
        name,
...
class DocumentSerializer {
  ...
  static ToDOCX = toDOCX2;
  static FromDOCX = fromDOCX2;
  static ToPDF = toPDF2;
