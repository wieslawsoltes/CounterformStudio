...
import { FlowDocument, elementFromJSON } from "./model.js";
import {
  parseMarkup,
  child,
  descendants,
...
function officeMarkup(n) {
  ...
  return `<${n.name}${Object.entries(n.attrs).map(([k, v]) => ` ${k}="${esc(v)}"`).join("")}>${n.children.map(officeMarkup).join("")}</${n.name}>`;
}
function normalizeFields(nodes) {
  ...
  for (const n of nodes) {
    const fld = n.name === "w:r" ? child(n, "w:fldChar") : void 0, kind = fld?.attrs["w:fldCharType"];
    if (kind === "begin") {
...
function groupTableOfContents(children) {
  ...
  for (let index = 0; index < children.length; index++) {
    const first = children[index], instruction = descendants(first, "w:instrText").map(textContent).join("").trim();
    if (first.name !== "w:p" || !/^TOC(?:\s|$)/i.test(instruction) || !descendants(first, "w:fldChar").some(
...
async function toDOCX(document) {
  ...
  const zip = new JSZip(), rels = [], numberings = [], mediaTypes = /* @__PURE__ */ new Set();
  let activeRels = rels;
  const extraTypes = [];
  ...
  for (const part of retainedParts) {
    ...
    if (part.Name.endsWith(".rels")) {
      ...
      const safe = descendants(relRoot, "Relationship").filter(
        (r) => retainedRelationship(r.attrs.Type ?? "") && (r.attrs.TargetMode === "External" ? r.attrs.Type?.endsWith("/hyperlink") && !!safeURL(r.attrs.Target) : retainedNames.has(
          originalPartName(
  ...
  const deletedXML = (nodes) => nodes.map(
    (n) => n.type === "Run" ? `<w:r>${runProperties(n.props)}<w:delText xml:space="preserve">${esc(n.text ?? "")}</w:delText></w:r>` : n.type === "LineBreak" ? "<w:r><w:br/></w:r>" : deletedXML(n.children ?? [])
  ).join("");
  const commentReviews = reviews.filter((a) => a.Kind === "Comment");
  for (const [index, a] of reviews.entries()) {
    const data = a.Data ?? {}, author = ` w:author="${esc(data.Author ?? "")}"`, date = data.CreatedAt ? ` w:date="${esc(data.CreatedAt)}"` : "";
    if (a.Kind === "Comment") {
  ...
  const revisionAttrs = (a) => ` w:id="${reviews.indexOf(a)}" w:author="${esc(a.Data?.Author ?? "")}"${a.Data?.CreatedAt ? ` w:date="${esc(a.Data.CreatedAt)}"` : ""}`;
  ...
  const rawRun = (text, props) => "<w:r>" + changedRunProperties(props, exportOffset) + text.split(/(\n|\t)/).map(
    (t) => t === "\n" ? "<w:br/>" : t === "	" ? "<w:tab/>" : `<w:t xml:space="preserve">${esc(t)}</w:t>`
  ).join("") + "</w:r>";
  ...
  const inline = (n, inherited = {}) => {
    ...
    if (n.props.Field) {
      ...
      return [
        ...
        "PAGEREF",
        "FILENAME"
      ].includes(type) ? `<w:fldSimple w:instr="${esc(instruction)}" w:dirty="${field.Dirty ? "true" : "false"}">${body}</w:fldSimple>` : body;
    }
    ...
    if (n.type === "Hyperlink") {
      const uri = safeURL(props.NavigateUri), body = (n.children ?? []).map((c) => inline(c, props)).join("");
    ...
    if (n.type === "Equation") {
      const prefix = emitMarkers(), at = exportOffset;
      if (reviewActive) exportOffset++;
      return prefix + wrapInsertion(equationToOMML(equationOptions(n)), at) + emitMarkers();
    }
    if (n.type === "Image") {
      ...
      const rowXML = rows.map((row) => {
        ...
        while (index < cells.length || [...active.keys()].some((key) => key >= column)) {
          ...
          if (mark && !mark.inserted) {
            ...
            for (const paragraph2 of descendants(tree, "w:p")) {
  ...
  const sectionProperties = (p) => {
    const geometry = pageSettings(p), margins = geometry.Padding;
    let refs = "";
  ...
  if (commentReviews.length) {
    ...
    const commentBody = commentReviews.map((a, index) => {
      ...
      const last = descendants(content2, "w:p").at(-1);
      if (last)
        last.attrs["w14:paraId"] = (index + 1).toString(16).padStart(8, "0").toUpperCase();
      return `<w:comment w:id="${index}" w:author="${esc(a.Data?.Author ?? "")}"${a.Data?.CreatedAt ? ` w:date="${esc(a.Data.CreatedAt)}"` : ""}>${officeMarkup(content2)}</w:comment>`;
    }).join("");
    activeRels = rels;
    zip.file(
      "word/comments.xml",
  ...
  if (pageStoryVariantEnabled(root.props, "EvenPage") || root.props.TrackChanges) {
...
async function fromDOCX(bytes) {
  ...
  const rootRels = parseOfficeXML(await read("_rels/.rels")), mainRel = descendants(rootRels, "Relationship").find(
  ...
  const runProps = (pr) => {
    ...
    if (child(pr, "w:u"))
      p.TextDecorations = val(child(pr, "w:u")) === "none" ? "None" : "Underline";
  ...
  const paragraphProps = (pr) => {
    const styleId = val(child(pr, "w:pStyle")), p = inheritedStyle(styleId);
    if (!pr) return p;
    if (child(pr, "w:bidi"))
      p.FlowDirection = bool(child(pr, "w:bidi")) ? "RightToLeft" : "LeftToRight";
    ...
    const outline = val(child(pr, "w:outlineLvl")) ?? val(child(child(styles.get(styleId ?? ""), "w:pPr"), "w:outlineLvl"));
    ...
    const spacing = child(pr, "w:spacing"), indent = child(pr, "w:ind");
  ...
  const sectionProps = (section) => {
    const p = {};
    if (!section) return p;
    const size = child(section, "w:pgSz"), margin = child(section, "w:pgMar"), cols = child(section, "w:cols");
    if (size) {
  ...
  const inlines = (nodes, inherited = {}) => nodes.flatMap((n) => {
    if (n.name === "w:r") {
      const pr = child(n, "w:rPr"), current = { ...inherited, ...runProps(pr) }, start = importOffset;
    ...
    if (n.name === "w:commentReference") {
      ...
      if (recordReview && !exists && !annotationStarts.has("w:commentRangeStart:" + cid)) {
        ...
        annotations.push({
          ...
          Data: {
            DocxId: cid,
            Text: comment ? descendants(comment, "w:p").map((p) => descendants(p, "w:t").map(textContent).join("")).join("\n") : "",
    ...
    if (n.name === "w:bookmarkEnd" || n.name === "w:commentRangeEnd") {
      ...
      if (recordReview && start) {
        const comment = comments.get(n.attrs["w:id"] ?? ""), data = bookmark ? { Name: start.Name ?? "Bookmark" } : {
          Text: comment ? descendants(comment, "w:p").map(
            (p) => descendants(p, "w:t").map(textContent).join("")
          ).join("\n") : "",
          Author: comment?.attrs["w:author"] ?? "",
          CreatedAt: comment?.attrs["w:date"] ?? "",
          Resolved: false,
    ...
    if (n.name === "w:hyperlink") {
      const rel = relationships.get(n.attrs["r:id"] ?? ""), uri = safeURL(
    ...
    if (n.name === "w:drawing") {
      ...
      if (textbox) {
        ...
        const extent2 = descendants(n, "wp:extent")[0], horizontal = descendants(n, "wp:positionH")[0], vertical = descendants(n, "wp:positionV")[0], alignH = textContent(
        ...
        ), wrap = descendants(n, "wp:wrapSquare")[0]?.attrs.wrapText, kind = descendants(n, "wp:docPr")[0]?.attrs.name?.startsWith(
          "Floater"
        ) ? "Floater" : "Figure";
        return [
          node(kind, content, {
            ...
            WrapDirection: descendants(n, "wp:wrapNone").length ? "None" : wrap === "left" ? "Left" : wrap === "right" ? "Right" : "Both"
          })
        ];
      }
      const blip = descendants(n, "a:blip")[0], src = images.get(blip?.attrs["r:embed"] ?? ""), extent = descendants(n, "wp:extent")[0], description = descendants(n, "wp:docPr")[0]?.attrs.descr ?? "";
      if (!src) {
        const opaque = captureOpaque(n);
  ...
  const convertBlocks = (children) => {
    ...
    for (const n of groupTableOfContents(children)) {
      if (n.name === "rtw:toc") {
        ...
        const pr = child(n, "w:pPr"), props = paragraphProps(pr), para = node(
          "Paragraph",
          inlines(normalizeFields(n.children), props),
          props
        ), numPr = child(pr, "w:numPr"), numId = val(child(numPr, "w:numId"));
        const changed = child(pr, "w:pPrChange");
        if (recordReview && changed) {
          const before = paragraphProps(child(changed, "w:pPr")), changes = nativePropertyChanges(before, props).map((c) => ({
            ...c,
            Scope: "Node",
            NodeId: para.id
          }));
          if (changes.length)
            annotations.push({
              Id: "docx-format-" + ++id,
              Kind: "Formatting",
              Start: importOffset - FlowDocument.FromJSON(node("FlowDocument", [para])).Text.length,
              End: importOffset,
              Data: {
                Author: changed.attrs["w:author"] ?? "",
                CreatedAt: changed.attrs["w:date"] ?? "",
        ...
        for (const row of n.children.filter((c) => c.name === "w:tr")) {
          const rowPr = child(row, "w:trPr"), rowDelete = child(rowPr, "w:del"), rowInsert = child(rowPr, "w:ins"), initialOffset = importOffset, initialParagraphs = importParagraphs, outerReview = recordReview, annotationIndex = annotations.length, importedRow = node("TableRow"), cells = [], touched = /* @__PURE__ */ new Set();
          let column = 0;
          if (rowDelete) recordReview = false;
          importedRow.children = cells;
          for (const cell of row.children.filter((c) => c.name === "w:tc")) {
            const pr = child(cell, "w:tcPr"), span = Math.max(1, Number(val(child(pr, "w:gridSpan"))) || 1), merge = child(pr, "w:vMerge"), existing = active.get(column), cellDelete = child(pr, "w:cellDel"), cellInsert = child(pr, "w:cellIns");
  ...
  const parsed = parseOfficeXML(main), body = descendants(parsed, "w:body")[0];
  ...
  for (const props of storyProps) {
    for (const ref of props.DocxStoryReferences ?? []) {
      ...
      const story = await loadStory(resolve(rel.Target)), content = descendants(
        story,
  ...
  for (const kind of ["Footnote", "Endnote"]) {
    ...
    defaults[kind + "s"] = descendants(story, "w:" + kind.toLowerCase()).filter((n) => !n.attrs["w:type"] && Number(n.attrs["w:id"]) > 0).map((n) => ({ Id: n.attrs["w:id"], Blocks: convertBlocks(n.children) }));
  }
