...
function fontName(props) {
  ...
  if (/courier|mono|consolas/.test(family))
    return bold ? italic ? StandardFonts.CourierBoldOblique : StandardFonts.CourierBold : italic ? StandardFonts.CourierOblique : StandardFonts.Courier;
  if (/times|serif|georgia|cambria/.test(family) && !/sans/.test(family))
    return bold ? italic ? StandardFonts.TimesRomanBoldItalic : StandardFonts.TimesRomanBold : italic ? StandardFonts.TimesRomanItalic : StandardFonts.TimesRoman;
...
class PDFEditor {
  ...
  async ReplaceTextOperator(pageIndex, operatorId, replacement, options = {}) {
    return this.editSource(
      (pdf) => replacePDFTextOperator(pdf, pageIndex, operatorId, replacement, options)
    );
  }
  ...
  async ReplaceSourceText(query, replacement, options = {}) {
    return this.editSource(
      (pdf) => replacePDFSourceText(pdf, query, replacement, options)
    );
  }
  editSource(action) {
