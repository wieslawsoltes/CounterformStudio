...
const run = (text, properties = "") => `<m:r>${properties ? `<m:rPr>${properties}</m:rPr>` : ""}<m:t xml:space="preserve">${esc(text)}</m:t></m:r>`;
...
function equationToOMML(options) {
  ...
  const convert = (n) => {
    ...
    const properties = (tag, values) => wrap(
      tag + "Pr",
      Object.entries(values).map(([key, value]) => `<m:${key} m:val="${esc(value)}"/>`).join("")
    );
    switch (name) {
      case "math":
      case "mrow":
...
function ommlToMathML(source) {
  const root = typeof source === "string" ? parseMarkup(source, true).children[0] : source;
  let count = 0;
  const convert = (n, depth = 0) => {
    ...
    switch (name) {
      ...
      case "fName":
        return body();
      case "r": {
        const t = get("t"), value = t ? textContent(t) : "";
      ...
      case "rad":
        return property("rad", "degHide", "0") === "1" || !get("deg") || !textContent(get("deg")).trim() ? math("msqrt", part("e")) : math("mroot", part("e") + part("deg"));
      case "sSup":
