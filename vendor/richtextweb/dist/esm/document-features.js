...
function updateDocumentFields(root, context = {}) {
  ...
  const originalText = plainText(root), originalBlocks = textBlocks(root), originalAnnotations = copy(root.props.Annotations ?? []);
  walk(root, (node) => {
...
class DocumentFeatures {
  ...
  InsertNote(kind, content) {
    ...
    const id = new Run().Id, key = kind === "Footnote" ? "Footnotes" : "Endnotes";
    const blocks = typeof content === "string" ? [new Paragraph(content).ToJSON()] : copy(content);
