...
__export(formats_exports, {
  ...
  PDFEditor: () => import_formats_pdf.PDFEditor,
  fromDOCX: () => import_formats_docx.fromDOCX,
  fromHTML: () => fromHTML,
  ...
  toDOCX: () => import_formats_docx.toDOCX,
  toHTML: () => toHTML,
  toMarkdown: () => toMarkdown,
  toPDF: () => import_formats_pdf.toPDF,
  toRTF: () => toRTF,
  toText: () => toText,
  toXAML: () => toXAML
});
...
var import_formats_docx = require("./formats-docx.js");
var import_formats_pdf = require("./formats-pdf.js");
var import_formats_docx2 = require("./formats-docx.js");
var import_formats_pdf2 = require("./formats-pdf.js");
let nextId = 0;
function makeNode(type, children = [], props = {}, text) {
  return {
    type,
...
const run = (text, props = {}) => makeNode("Run", [], props, text);
const paragraph = (inlines = [], props = {}) => makeNode("Paragraph", inlines, props);
const docFromNodes = (blocks, props = {}) => import_model.FlowDocument.FromJSON(
...
const isColor = (value) => /^(?:#[a-f0-9]{3,8}|rgba?\([\d.% ,/]+\)|hsla?\([\d.% ,/]+\)|[a-z]+)$/i.test(
  String(value)
);
...
function thickness(value) {
  ...
  if (typeof value === "string" && value.includes(",")) {
    const v = value.split(",").map((v2) => cssLength(v2.trim())).filter(Boolean);
    return (v.length === 4 ? [v[1], v[2], v[3], v[0]] : v.length === 2 ? [v[1], v[0]] : v).join(" ");
  }
...
function styles(props) {
  ...
  const decorations = String(props.TextDecorations ?? "").toLowerCase().replace(/strikethrough/g, "line-through");
  ...
  if (/^(?:left|center|right|justify|start|end)$/i.test(
  ...
  add(
    "line-height",
    typeof props.LineHeight === "number" && props.LineHeight > 0 && props.LineHeight <= 4 ? String(props.LineHeight) : cssLength(props.LineHeight)
  );
  ...
  if (/^(?:pre|pre-wrap|pre-line|normal|break-spaces)$/.test(
...
const floatingPropertyNames = /* @__PURE__ */ new Set([
  "Width",
  "Height",
  "WrapStyle",
  "HorizontalAlignment",
...
function floatingProperties(props) {
  if (!props || typeof props !== "object" || Array.isArray(props)) return {};
  return Object.fromEntries(
    Object.entries(props).filter(([key]) => floatingPropertyNames.has(key))
  );
}
function safeFloatingProperties(props, kind) {
  const result = {};
  for (const [key, value] of Object.entries(floatingProperties(props))) {
    try {
      if ((key === "Width" || key === "Height") && value && typeof value === "object") {
        if (kind !== "Figure" || Array.isArray(value) || typeof value.Value !== "number" || !Object.values(import_model.FigureUnitType).includes(value.FigureUnitType))
          continue;
        result[key] = new import_model.FigureLength(
          value.Value,
          value.FigureUnitType
        ).toJSON();
      } else if (key === "HorizontalAnchor" || key === "VerticalAnchor" || key === "WrapDirection") {
        if (kind !== "Figure") continue;
        const allowed = key === "HorizontalAnchor" ? import_model.FigureHorizontalAnchor : key === "VerticalAnchor" ? import_model.FigureVerticalAnchor : import_model.WrapDirection;
        ...
        Object.assign(result, (0, import_floating_layout.normalizeFloatingLayout)({ [key]: value }));
      }
    } catch {
    }
  }
...
function readFloatingProperties(raw, kind) {
...
function floatingStyleProperties(props, kind) {
  const result = { ...props };
  delete result.Width;
  ...
  for (const key of ["Width", "Height"]) {
    ...
    else if (kind === "Figure" && typeof value === "string" && /^\d+(?:\.\d+)?%$/.test(value)) {
      const fraction = parseFloat(value) / 100;
      if (fraction <= 1)
        result[key] = new import_model.FigureLength(fraction, "Content").toJSON();
    }
  }
  return result;
}
function floatingStyles(props) {
  const wrap = props.WrapStyle ?? "Square", align = props.HorizontalAlignment ?? (/Left/.test(props.HorizontalAnchor) ? "Left" : "Right");
  const result = [
  ...
  for (const name of ["Width", "Height"]) {
    const value = props[name], number = typeof value === "number" ? value : value?.FigureUnitType === "Pixel" ? value.Value : void 0;
    if (Number.isFinite(number) && number >= 0)
      result.push(`${name.toLowerCase()}:${Math.min(2e4, number)}px`);
    else if (["Page", "Content"].includes(value?.FigureUnitType) && Number.isFinite(value.Value))
      result.push(
  ...
  for (const [name, css] of [
    ...
    if (Number.isFinite(props[name]))
      result.push(`${css}:${Math.max(-2e4, Math.min(2e4, props[name]))}px`);
...
function htmlNode(node, phrasingBlocks = false) {
  const p = node.props ?? {}, css = styles(p), attr = css ? ` style="${(0, import_formats_markup.escapeMarkup)(css)}"` : "";
  const floating = node.type === "Figure" || node.type === "Floater";
  const body = (node.children ?? []).map((child) => htmlNode(child, phrasingBlocks || floating)).join("");
  const wrap = (tag, extra = "") => phrasingBlocks && floatingBlockDisplays[tag] ? `<span data-rt-block="${tag}" style="display:${floatingBlockDisplays[tag]}${css ? ";" + (0, import_formats_markup.escapeMarkup)(css) : ""}"${extra}>${body}</span>` : `<${tag}${attr}${extra}>${body}</${tag}>`;
  switch (node.type) {
    case "FlowDocument":
    ...
    case "Paragraph":
      return wrap(
        Number(p.HeadingLevel) >= 1 && Number(p.HeadingLevel) <= 6 ? "h" + Number(p.HeadingLevel) : "p"
      );
    case "Run": {
      const text = node.text ?? "", preserve = /[\t\r\n]| {2,}|^ | $/.test(text), runCss = css + (preserve ? (css ? ";" : "") + "white-space:pre-wrap" : "");
      return runCss ? `<span style="${(0, import_formats_markup.escapeMarkup)(runCss)}">${(0, import_formats_markup.escapeMarkup)(text)}</span>` : (0, import_formats_markup.escapeMarkup)(text);
    }
    case "Bold":
      return wrap("strong");
    case "Italic":
    ...
    case "Span":
      return wrap("span");
    case "Hyperlink": {
      const href = (0, import_formats_markup.safeURL)(p.NavigateUri);
      return href ? wrap("a", ` href="${(0, import_formats_markup.escapeMarkup)(href)}" rel="noopener noreferrer"`) : wrap("span");
    }
    case "LineBreak":
      return "<br>";
    case "List": {
      ...
      return wrap(
        ordered ? "ol" : "ul",
        ordered && Number.isFinite(Number(p.StartIndex)) ? ` start="${Math.max(1, Number(p.StartIndex))}"` : ""
      );
    }
    case "ListItem":
      return wrap("li");
    ...
    case "TableCell":
      return wrap(
        "td",
        ` colspan="${Math.max(1, Math.min(1e3, Number(p.ColumnSpan) || 1))}" rowspan="${Math.max(1, Math.min(1e3, Number(p.RowSpan) || 1))}"`
      );
    case "Equation": {
      const options = (0, import_equations.equationOptions)(node), rendered = (0, import_equations.renderEquation)(options);
      return `<span data-rt-equation="${options.Format}" data-rt-source="${(0, import_formats_markup.escapeMarkup)(options.Source)}" data-rt-display="${!!options.DisplayMode}" role="math" aria-label="${(0, import_formats_markup.escapeMarkup)(options.AlternativeText || rendered.Text)}" style="${(0, import_formats_markup.escapeMarkup)(css)};display:${options.DisplayMode ? "block;text-align:center;margin:.6em 0" : "inline-block"};line-height:normal;direction:ltr">${rendered.SVG}</span>`;
    }
    case "Image": {
      const src = (0, import_formats_markup.safeURL)(p.Source, true);
      const layout = safeFloatingProperties(p, "Image");
      const floatingImage = Object.keys(layout).some(
        (key) => key !== "Width" && key !== "Height"
      );
      const imageCss = [css, floatingImage ? floatingStyles(layout) : ""].filter(Boolean).join(";");
      return src ? `<img${imageCss ? ` style="${(0, import_formats_markup.escapeMarkup)(imageCss)}"` : ""}${Object.keys(layout).length ? ` data-rt-layout="${(0, import_formats_markup.escapeMarkup)(JSON.stringify(layout))}"` : ""} src="${(0, import_formats_markup.escapeMarkup)(src)}" alt="${(0, import_formats_markup.escapeMarkup)(p.AlternativeText ?? "")}">` : (0, import_formats_markup.escapeMarkup)(p.AlternativeText ?? "");
    }
    case "Figure":
    case "Floater":
      return `<span data-rt-floating="${node.type}" data-rt-layout="${(0, import_formats_markup.escapeMarkup)(JSON.stringify(safeFloatingProperties(p, node.type)))}" style="${(0, import_formats_markup.escapeMarkup)(css + ";" + floatingStyles(p))}">${body}</span>`;
    case "InlineUIContainer":
      return wrap("span");
    case "BlockUIContainer":
      return wrap("div");
    default:
      return body || (0, import_formats_markup.escapeMarkup)(node.text ?? "");
  }
}
function toHTML(doc) {
  return htmlNode(doc.ToJSON());
}
function parsedLength(value) {
  return /^-?[\d.]+(?:px|pt)?$/.test(value) ? parseFloat(value) * (value.endsWith("pt") ? 96 / 72 : 1) : value;
}
function parseStyle(source) {
  ...
  for (const declaration of source.split(";")) {
    ...
    if (colon < 0) continue;
    const key = declaration.slice(0, colon).trim().toLowerCase(), value = declaration.slice(colon + 1).trim().replace(/\s*!important$/i, "");
    ...
    if (key === "white-space" && /^(?:pre|pre-wrap|pre-line|normal|break-spaces)$/.test(value)) {
    ...
    if (["FontSize", "Width", "Height", "LineHeight"].includes(prop)) {
      if (/^-?[\d.]+(?:px|pt|em|rem|%)?$/.test(value)) {
        ...
        if (prop === "FontSize") {
          const size = typeof parsed === "number" ? parsed : parseFloat(parsed) * (parsed.endsWith("%") ? 0.16 : 16);
          if (Number.isFinite(size) && size > 0) props[prop] = size;
        } else props[prop] = parsed;
      }
    } else if (prop === "Margin" || prop === "Padding") {
      const parts = value.split(/\s+/);
      if (parts.length <= 4 && parts.every((v) => /^-?[\d.]+(?:px|pt)?$/.test(v))) {
        const v = parts.map((v2) => Number(parsedLength(v2)));
        props[prop] = {
          Top: v[0],
          Right: v[1] ?? v[0],
          Bottom: v[2] ?? v[0],
...
const suppressedHTML = /* @__PURE__ */ new Set([
  "script",
  "style",
  "iframe",
  "object",
...
const htmlBlocks = /* @__PURE__ */ new Set([
  "p",
  "div",
  "section",
  "article",
...
function htmlInlines(nodes, preserveSpace = false) {
  const result = [];
  for (const node of nodes) {
    const name = node.name, p = parseStyle(node.attrs.style ?? "");
    if (name === "#text") {
      const text = preserveSpace ? node.text : node.text.replace(/[\t\r\n ]+/g, " ");
      if (text) result.push(run(text));
      continue;
    }
    if (suppressedHTML.has(name)) continue;
    if (node.attrs["data-rt-equation"] || name === "math") {
      const format = node.attrs["data-rt-equation"] === "latex" ? "latex" : "mathml";
      const source = name === "math" ? (0, import_equations.sanitizeMathML)((0, import_equations.serializeMathNode)(node)) : node.attrs["data-rt-source"] ?? "";
      (0, import_equations.renderEquation)({
        Source: source,
        Format: format,
    ...
    if (node.attrs["data-rt-floating"] === "Figure" || node.attrs["data-rt-floating"] === "Floater") {
      const kind = node.attrs["data-rt-floating"];
      const restored = (item) => ({
        ...item,
        name: item.name === "span" && Object.prototype.hasOwnProperty.call(
          floatingBlockDisplays,
          item.attrs["data-rt-block"]
        ) ? item.attrs["data-rt-block"] : item.name,
    ...
    if (name === "img") {
      const source = (0, import_formats_markup.safeURL)(node.attrs.src, true);
      if (source)
        result.push(
          makeNode("Image", [], {
            ...p,
            ...
            ...node.attrs.width ? { Width: Number(node.attrs.width) || void 0 } : {},
            ...node.attrs.height ? { Height: Number(node.attrs.height) || void 0 } : {}
          })
        );
    ...
    const children = htmlInlines(
      node.children,
      name === "code" || name === "pre" || (p.WhiteSpace ? ["pre", "pre-wrap", "break-spaces"].includes(p.WhiteSpace) : preserveSpace)
    );
    const types = {
      b: "Bold",
      strong: "Bold",
    ...
    if (name === "a") {
      const uri = (0, import_formats_markup.safeURL)(node.attrs.href);
...
function htmlBlockNodes(nodes, preserveSpace = false) {
  const result = [];
  let pending = [];
  const flush = () => {
  ...
  for (const node of nodes) {
    ...
    flush();
    const p = parseStyle(node.attrs.style ?? ""), name = node.name, nodePreserveSpace = p.WhiteSpace ? ["pre", "pre-wrap", "break-spaces"].includes(p.WhiteSpace) : preserveSpace;
    ...
    if (name === "blockquote") {
      ...
      result.push(
        makeNode(
          "List",
          node.children.filter((n) => n.name === "li").map(
            (n) => makeNode(
              "ListItem",
      ...
      const gather = (n) => {
        if (n.name === "tr")
          rows.push(
            makeNode(
              "TableRow",
              n.children.filter((c) => c.name === "td" || c.name === "th").map(
                (c) => makeNode(
                  "TableCell",
    ...
    else if (name === "div" || name === "section" || name === "article" || name === "main" || name === "header" || name === "footer") {
...
function fromHTML(html) {
  return docFromNodes(htmlBlockNodes((0, import_formats_markup.parseMarkup)(html).children));
}
...
function markdownInline(node) {
  ...
  switch (node.type) {
    ...
    case "Underline":
      return "<u>" + inner + "</u>";
    case "Hyperlink": {
      const uri = (0, import_formats_markup.safeURL)(node.props.NavigateUri);
      return uri ? `[${inner}](${uri.replace(/[()\s]/g, (c) => encodeURIComponent(c))})` : inner;
    }
    case "Equation": {
      const options = (0, import_equations.equationOptions)(node);
      if (options.Format !== "mathml" && !options.Source.includes("\n"))
        return `$${options.Source}$`;
      return `<span data-rt-equation="${options.Format}" data-rt-source="${(0, import_formats_markup.escapeMarkup)(options.Source)}" data-rt-display="${!!options.DisplayMode}">[equation]</span>`;
    }
    case "Image": {
      const uri = (0, import_formats_markup.safeURL)(node.props.Source, true);
      return uri ? `![${escape(String(node.props.AlternativeText ?? ""))}](${uri.replace(/[()\s]/g, (c) => encodeURIComponent(c))})` : "";
    }
    case "Figure":
    case "Floater":
...
function markdownBlocks(nodes, depth = 0) {
  return nodes.map((node) => {
    if (node.type === "Paragraph" && node.children?.length === 1 && node.children[0].type === "Equation" && node.children[0].props.DisplayMode && node.children[0].props.EquationFormat !== "mathml")
      return `$$
${node.children[0].props.EquationSource}
$$`;
    if (node.type === "Paragraph")
      return (node.props.HeadingLevel ? "#".repeat(
    ...
    if (node.type === "List")
      return (node.children ?? []).map((item, i) => {
        ...
        return prefix + markdownBlocks(item.children ?? [], depth + 1).replace(
          /\n/g,
    ...
    if (node.type === "Table") {
      ...
      const lines = rows.map(
        (row) => "| " + (row.children ?? []).map(
          (cell) => markdownBlocks(cell.children ?? []).replace(/\|/g, "\\|").replace(/\n+/g, "<br>")
        ).join(" | ") + " |"
      );
      lines.splice(
        1,
    ...
    return node.children ? markdownBlocks(node.children, depth) : markdownInline(node);
  }).join("\n\n");
}
function toMarkdown(doc) {
  return markdownBlocks(doc.ToJSON().children ?? []);
}
const markdownParser = new import_marked.Marked({ gfm: true, async: false });
const equationMarkup = (source, display) => `<span data-rt-equation="latex" data-rt-source="${(0, import_formats_markup.escapeMarkup)(source)}" data-rt-display="${display}">[equation]</span>`;
markdownParser.use({
  extensions: [
    {
      name: "blockEquation",
...
function fromMarkdown(markdown) {
  ...
  return fromHTML(markdownParser.parse(markdown, { async: false }));
}
...
const numericProperties = /* @__PURE__ */ new Set([
  "FontSize",
  "LineHeight",
  "PageWidth",
  "PageHeight",
...
function xamlNode(node, root = false) {
  const props = Object.entries(node.props ?? {}).filter(([key, value]) => knownProperties.has(key) && value != null).map(([key, value]) => {
    if (node.type === "Figure" && (key === "Width" || key === "Height") && typeof value === "object")
      value = new import_model.FigureLength(value.Value, value.FigureUnitType).ToString();
    ...
    return ` ${key}="${(0, import_formats_markup.escapeMarkup)(value)}"`;
  }).join("");
  const attrs = (root ? ' xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xml:space="preserve"' : "") + props;
  const columns = node.type === "Table" && Array.isArray(node.props.Columns) && node.props.Columns.length ? `<Table.Columns>${node.props.Columns.map((column) => xamlNode(column)).join("")}</Table.Columns>` : "";
  return `<${node.type}${attrs}>${columns}${node.type === "Run" ? (0, import_formats_markup.escapeMarkup)(node.text ?? "") : (node.children ?? []).map((n) => xamlNode(n)).join("")}</${node.type}>`;
}
function toXAML(doc) {
  return xamlNode(doc.ToJSON(), true);
}
function fromXAML(xaml) {
  const root = (0, import_formats_markup.parseMarkup)(xaml, true);
  const convert = (node, parent = "", inheritedPreserve = false) => {
    const name = node.name.split(":").pop(), preserve = node.attrs["xml:space"] === "preserve" || node.attrs["xml:space"] !== "default" && inheritedPreserve;
    if (name === "#text")
      return node.text?.trim() || preserve && /^(?:Paragraph|Span|Bold|Italic|Underline|Hyperlink)$|\.Inlines$/.test(
        parent
      ) ? [run(node.text ?? "")] : [];
    ...
    for (const [key, value] of Object.entries(node.attrs)) {
      ...
      if (key === "Source" || key === "NavigateUri") {
        const uri = (0, import_formats_markup.safeURL)(value, key === "Source");
        if (uri) props[key] = uri;
      } else if (name === "Figure" && (key === "Width" || key === "Height")) {
        props[key] = import_model.FigureLength.Parse(value).toJSON();
        ...
        if (parts.every(Number.isFinite))
          props[key] = parts.length === 1 ? parts[0] : {
            Left: parts[0],
            Top: parts[1],
    ...
    if (name === "Table") {
      ...
      if (propertyNodes.length) {
        props.Columns = propertyNodes.flatMap(
          (property) => property.children.flatMap(
            (child) => convert(child, "Table.Columns", preserve)
          )
        );
        if (props.Columns.some(
          (column) => column.type !== "TableColumn"
        ))
          throw new TypeError("Table.Columns requires TableColumn children.");
      }
    }
    const text = name === "Run" ? node.attrs.Text ?? (0, import_formats_markup.textContent)(node) : void 0;
    return [
      makeNode(
        name,
        name === "Run" ? [] : node.children.flatMap((child) => convert(child, name, preserve)),
        props,
        text
      )
    ];
  ...
  return import_model.FlowDocument.FromJSON(document);
}
const rtfEscape = (value) => value.replace(/[\\{}\n\r\t]|[^\x20-\x7e]/g, (char) => {
  if (char === "\n") return "\\line ";
  if (char === "\r") return "";
  if (char === "	") return "\\tab ";
...
function toRTF(doc) {
  ...
  const emit = (node) => {
    ...
    if (node.type === "Bold" || p.FontWeight != null)
      prefix += node.type === "Bold" || /bold|[6-9]00/i.test(String(p.FontWeight)) ? "\\b " : "\\b0 ";
    if (node.type === "Italic" || p.FontStyle != null)
      prefix += node.type === "Italic" || /italic/i.test(String(p.FontStyle)) ? "\\i " : "\\i0 ";
    if (node.type === "Underline" || p.TextDecorations != null)
      prefix += node.type === "Underline" || /underline/i.test(String(p.TextDecorations)) ? "\\ul " : "\\ulnone ";
    if (/line-through|strikethrough/i.test(String(p.TextDecorations)))
      prefix += "\\strike ";
    if (p.BaselineAlignment)
      prefix += p.BaselineAlignment === "Superscript" ? "\\super " : p.BaselineAlignment === "Subscript" ? "\\sub " : "\\nosupersub ";
    if (p.FontFamily)
      prefix += "\\f" + fonts.indexOf(String(p.FontFamily)) + " ";
    const color = colorValue(p.Foreground), background = colorValue(p.Background);
    ...
    if (p.TextAlignment)
      prefix += { center: "\\qc ", right: "\\qr ", justify: "\\qj " }[String(p.TextAlignment).toLowerCase()] ?? "\\ql ";
    if (node.type === "LineBreak") return "\\line ";
    if (node.type === "Image")
      return rtfEscape(String(p.AlternativeText ?? ""));
    let body = node.type === "Run" ? rtfEscape(node.text ?? "") : (node.children ?? []).map(emit).join("");
    if (node.type === "Paragraph") body += "\\par\n";
    return "{" + prefix + body + "}";
  };
  return "{\\rtf1\\ansi\\deff0\\uc1{\\fonttbl" + fonts.map(
    (font, i) => "{\\f" + i + " " + rtfEscape(font.replace(/;/g, "")) + ";}"
  ).join("") + "}{\\colortbl;" + colors.map(
    (color) => "\\red" + parseInt(color.slice(0, 2), 16) + "\\green" + parseInt(color.slice(2, 4), 16) + "\\blue" + parseInt(color.slice(4, 6), 16) + ";"
  ).join("") + "}\n" + emit(doc.ToJSON()) + "}";
}
function fromRTF(rtf) {
  ...
  for (const segment of destination("colortbl").split(";").slice(0, -1)) {
    const red = segment.match(/\\red(\d+)/)?.[1], green = segment.match(/\\green(\d+)/)?.[1], blue = segment.match(/\\blue(\d+)/)?.[1];
    colors.push(
      red != null && green != null && blue != null ? "#" + [red, green, blue].map(
        (v) => Math.max(0, Math.min(255, Number(v))).toString(16).padStart(2, "0")
      ).join("") : ""
    );
  }
  let state = { props: {}, paragraph: {}, skip: false, uc: 1 };
  const stack = [], blocks = [];
  let inlines = [], buffer = "", fallback = 0;
  const flush = () => {
  ...
  const skipDestinations = /* @__PURE__ */ new Set([
    "fonttbl",
    "colortbl",
    "stylesheet",
    "info",
  ...
  for (let i = 0; i < rtf.length; ) {
    ...
    if (word === "uc") state.uc = Math.max(0, Math.min(16, n));
      ...
      state.props.BaselineAlignment = word === "super" ? "Superscript" : word === "sub" ? "Subscript" : "Baseline";
    else if (word === "plain") state.props = {};
    else if (word === "pard") state.paragraph = {};
    else if (["ql", "qc", "qr", "qj"].includes(word))
      state.paragraph.TextAlignment = { ql: "Left", qc: "Center", qr: "Right", qj: "Justify" }[word];
...
class DocumentSerializer {
  ...
  static ToDOCX = import_formats_docx2.toDOCX;
  static FromDOCX = import_formats_docx2.fromDOCX;
  static ToPDF = import_formats_pdf2.toPDF;
  static Serialize(document, format = "json") {
    switch (format) {
      case "text":
  ...
  static Deserialize(value, format = "json") {
    switch (format) {
      case "text":
      ...
      case "json":
        return import_model.FlowDocument.FromJSON(JSON.parse(value));
      default:
        throw new Error("Unsupported format: " + format);
    }
  }
