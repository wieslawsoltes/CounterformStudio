...
var equation_control_exports = {};
__export(equation_control_exports, {
  EquationEditor: () => EquationEditor,
  registerEquationEditor: () => registerEquationEditor
});
module.exports = __toCommonJS(equation_control_exports);
...
class EquationEditor extends HTMLElementBase {
  value = {
    Source: "x",
    Format: "latex",
    DisplayMode: false,
  ...
  ConvertFormat(format) {
    ...
    let current = null;
    try {
      current = (0, import_equations.renderEquation)(this.value);
    } catch {
    }
    const source = current ? format === "mathml" ? current.MathML : (0, import_equations.mathMLToLaTeX)(current.MathML) : this.Source;
    const next = { ...this.value, Source: source, Format: format };
    (0, import_equations.renderEquation)(next);
  ...
  Validate() {
    clearTimeout(this.timer);
    try {
      this.result = (0, import_equations.renderEquation)(this.value);
      this.tokens = (0, import_equations.equationTokens)(this.result.MathML);
      this.error = "";
    } catch (error) {
      this.result = null;
      this.tokens = [];
  ...
  get SelectedMatrix() {
    ...
    try {
      return (0, import_equations.equationMatrixAt)(
        this.result.MathML,
        this.tokens[this.selected].Path
      );
    } catch {
  ...
  EditMatrix(operation) {
    if (this.locked) return;
    const selected = this.validSelection();
    const source = (0, import_equations.editEquationMatrix)(
      this.result.MathML,
      selected.Path,
      operation
    );
  ...
  InsertToken(text, before = false) {
    if (this.locked) return;
    const selected = this.validSelection();
    this.change(
      {
        ...this.value,
        Source: (0, import_equations.insertEquationToken)(
          this.result.MathML,
          selected.Path,
          text,
          before
  ...
  DeleteToken() {
    ...
    this.change(
      {
        ...this.value,
        Source: (0, import_equations.deleteEquationToken)(this.result.MathML, selected.Path),
        Format: "mathml"
      },
      true
    );
  ...
  InsertTemplate(name) {
    const template = import_equations.EquationTemplates.find((item) => item.Name === name);
  ...
  change(next, rebuild = false, deferred = false) {
  ...
  render() {
    ...
    const d = this.ownerDocument, style = d.createElement("style");
    ...
    for (const category of [
      ...new Set(import_equations.EquationTemplates.map((item) => item.Category))
    ]) {
      const group = d.createElement("optgroup");
      group.label = category;
      import_equations.EquationTemplates.filter((item) => item.Category === category).forEach(
    ...
    hint.className = "hint";
    hint.textContent = "Select a symbol in the preview, or use the token selector. Structural tools wrap the selected symbol. Visual edits use editable MathML.";
    ...
    this.tokenSelect.onchange = () => this.SelectToken(Number(this.tokenSelect.value));
    ...
    const display = d.createElement("label"), check = d.createElement("input");
    check.type = "checkbox";
    check.checked = this.DisplayMode;
    check.onchange = () => this.change({ ...this.value, DisplayMode: check.checked });
    ...
    format.onchange = () => {
      try {
        this.ConvertFormat(format.value);
      } catch (error) {
        format.value = this.Format;
        if (this.status)
    ...
    alt.className = "alt";
    alt.placeholder = "Describe the equation for readers using assistive technology";
    ...
    if (this.locked)
      this.shadowRoot.querySelectorAll("input,select,textarea,button").forEach((control) => control.disabled = true);
    this.renderPreview();
  }
  ...
  renderPreview() {
    ...
    const redo = this.shadowRoot?.querySelector(
      '[data-action="redo"]'
    );
  ...
  updateTokenSelection() {
    ...
    this.shadowRoot?.querySelectorAll("[data-matrix]").forEach((button) => {
      button.disabled = this.locked || !matrix || button.dataset.matrix === "DeleteRow" && matrix.Rows === 1 || button.dataset.matrix === "DeleteColumn" && matrix.Columns === 1;
    });
    ...
    this.preview?.querySelectorAll(
      ...
      (node, i) => node.toggleAttribute("data-selected", i === this.selected)
    );
  }
}
function registerEquationEditor(registry = globalThis.customElements) {
  if (registry && !registry.get("rich-equation-editor"))
    registry.define("rich-equation-editor", EquationEditor);
}
//# sourceMappingURL=equation-control.js.map
