...
class TextRange {
  ...
  withEngine(action) {
    ...
    try {
      action(engine);
      if (!this.owner) {
        this._start = new import_model.TextPointer(
          engine.Document,
          engine.Selection.Start.Offset
        );
        this._end = new import_model.TextPointer(
          engine.Document,
...
class TextSelection extends TextRange {
  constructor(engine) {
    super(
      new import_model.TextPointer(engine.Document, 0),
      new import_model.TextPointer(engine.Document, 0)
    );
    this.owner = engine;
  }
  get Start() {
    return new import_model.TextPointer(this.owner.Document, this.owner.SelectionStart);
  }
  get End() {
    return new import_model.TextPointer(this.owner.Document, this.owner.SelectionEnd);
  }
  Select(start, end) {
...
function propertyInRange(root, start, end, name, defaultValue) {
  const list = (0, import_engine_tree.leaves)(root);
...
class RichTextEngine {
  _document;
  Selection;
  Changed = new import_model.EventDispatcher();
  SelectionChanged = new import_model.EventDispatcher();
  subscription;
  undoStack = [];
  redoStack = [];
  typing = {};
  ...
  constructor(document = new import_model.FlowDocument()) {
  ...
  CaptureSelectionState() {
    return {
      Start: this.start,
      End: this.end,
      TypingProperties: (0, import_engine_tree.clone)(this.typing)
    };
  }
  RestoreSelectionState(state) {
    ...
    this.typing = (0, import_engine_tree.clone)(state.TypingProperties);
    this.emitSelection();
  }
  ...
  get HistoryStatistics() {
    return {
      ...
      RetainedBytes: [...this.undoStack, ...this.redoStack].reduce(
        (sum, item) => sum + (0, import_history.PatchByteLength)(item.patch),
        0
      )
    };
  }
  get Annotations() {
    return (0, import_engine_tree.clone)(this.Document.ToJSON().props.Annotations ?? []);
  }
  assertLive() {
  ...
  ReplaceDocument(document, options) {
    this.mutate(
      (root) => {
        const replacement = (0, import_engine_tree.clone)(document.ToJSON());
  ...
  ApplyRemoteDocument(document, options) {
    ...
    try {
      ...
      if (error instanceof import_history.DocumentObserverError) this.ClearUndo();
      throw error;
    } finally {
      this.historySuppressed--;
    }
  ...
  BeginChange() {
    ...
    if (this.depth++ === 0) {
      ...
      this.batchTokens = Array.from(
        { length: (0, import_engine_tree.plainText)(this.batch.document).length },
        (_, index) => index
      );
      ...
      if (this.batch && JSON.stringify(this.batch.document) !== JSON.stringify(this.Document.ToJSON()))
        this.record(
          ...
          this.batchTokensValid && this.batchTokens ? spansFromTokens(
            this.batchTokens,
            (0, import_engine_tree.plainText)(this.batch.document),
            this.Document.Text
          ) : void 0
        );
      this.batch = void 0;
  ...
  Undo() {
    ...
    try {
      ...
      (0, import_history.ApplyDocumentPatch)(this.Document, (0, import_history.InvertDocumentPatch)(entry.patch));
    } catch (error) {
      if (error instanceof import_history.DocumentObserverError) observerError = error;
      else {
  ...
  Redo() {
    ...
    try {
      ...
      (0, import_history.ApplyDocumentPatch)(this.Document, entry.patch);
    } catch (error) {
      if (error instanceof import_history.DocumentObserverError) observerError = error;
      else {
        this.Document.SetPendingTextChanges([]);
  ...
  cursor() {
    return { start: this.start, end: this.end, typing: (0, import_engine_tree.clone)(this.typing) };
  }
  setCursor(state) {
    this.start = state.start;
    this.end = state.end;
    this.typing = (0, import_engine_tree.clone)(state.typing);
  }
  snapshot() {
    return { ...this.cursor(), document: this.Document.ToJSON() };
  }
  record(state, after = this.Document.ToJSON(), textChanges) {
    const patch = (0, import_history.CreateDocumentPatch)(state.document, after);
  ...
  mutate(action, mapStructure = true, exactTextChange, review) {
    this.assertLive();
    const before = this.snapshot(), root = (0, import_engine_tree.clone)(before.document);
    try {
      action(root);
      ...
      if (review && this.TrackChanges && (review.Kind !== "Formatting" || this.TrackFormatting) && !this.reviewSuppressed) {
        const old = (0, import_engine_tree.clone)(before.document), next = (0, import_engine_tree.clone)(root);
        delete old.props.Annotations;
        delete next.props.Annotations;
        const patch = (0, import_history.CreateDocumentPatch)(old, next);
        if (patch)
          (root.props.Annotations ??= []).push({
            Id: (0, import_engine_tree.uid)(),
            Kind: review.Kind,
            Start: Math.min(this.start, (0, import_engine_tree.plainText)(root).length),
            End: Math.min(this.end, (0, import_engine_tree.plainText)(root).length),
            Data: {
              Author: this.CurrentAuthor,
              CreatedAt: (/* @__PURE__ */ new Date()).toISOString(),
              Operation: review.Operation,
              RestorePatch: (0, import_history.InvertDocumentPatch)(patch),
              StructureAnchors: captureStructureAnchors(
                root,
                (0, import_history.InvertDocumentPatch)(patch)
              ),
              ...(0, import_engine_tree.clone)(review.Data ?? {})
            }
          });
      }
      const document = import_model.FlowDocument.FromJSON(root);
      if (JSON.stringify(before.document) !== JSON.stringify(document.ToJSON())) {
        ...
        const exact = spans && document.Text.length === (0, import_engine_tree.plainText)(before.document).length + spans.reduce(
        ...
        } else if ((0, import_engine_tree.plainText)(before.document) !== document.Text && this.depth)
          this.batchTokensValid = false;
        (0, import_history.ReconcileDocument)(this.Document, document.ToJSON());
      ...
      if (error instanceof import_history.DocumentObserverError) throw error;
  ...
  InsertText(text) {
    ...
    this.mutate(
      (root) => {
        const from = this.start, to = this.end, previousLength = (0, import_engine_tree.plainText)(root).length;
        const tracked = this.TrackChanges && !this.reviewSuppressed;
        const deletion = tracked && to > from ? captureDeletion(root, from, to) : void 0;
        (0, import_engine_tree.deleteRange)(root, from, to);
        const position = Math.min(from, (0, import_engine_tree.plainText)(root).length);
        this.start = this.end = text ? (0, import_engine_tree.insertText)(root, position, text, props) : position;
        (0, import_engine_tree.mapMetadata)(
          root,
          from,
          to,
          (0, import_engine_tree.plainText)(root).length - previousLength + to - from
        );
        if (tracked) {
  ...
  tryDirectTextEdit(text) {
    ...
    const segment = segments.find(
      (item) => item.Element instanceof import_model.Run && item.TextStart < this.start && item.TextEnd >= this.end
    ) ?? segments.find(
      (item) => item.Element instanceof import_model.Run && item.TextStart === this.start && item.TextEnd >= this.end && item.Context === "Text"
    );
    if (!segment || !(segment.Element instanceof import_model.Run) || segment.Context !== "Text")
      return false;
    ...
    let change = (0, import_history.CreateDocumentPatch)(oldNode, newNode).Change;
    ...
    try {
      ...
      this.trimUndo();
      this.emitSelection();
      throw error instanceof import_history.DocumentObserverError ? error : new import_history.DocumentObserverError([error]);
    }
    this.trimUndo();
    this.emitSelection();
    return true;
  ...
  RejectRevision(id) {
        ...
        if (Array.isArray(item.Data.PropertyChanges)) {
          ...
          for (const change of [...changes].reverse()) {
            ...
            if (change.Scope === "Inline")
              (0, import_engine_tree.formatRange)(root, change.Start, change.End, change.Name, value);
            else
              setLocalProperty(
          ...
          for (const change of item.Data.StructureChanges) {
            ...
            parent.children.splice(index, removed.length, ...(0, import_engine_tree.clone)(inserted));
          }
        } else if (item.Data.RestorePatch) {
          const restored = (0, import_history.ApplyPatchToJSON)(
            root,
          ...
          if ((0, import_engine_tree.plainText)(root).slice(item.Start, item.End) !== item.Data.Text || !Number.isInteger(item.Data.SourceStart))
            throw new Error(
              "Revision conflict: moved content no longer matches its source."
            );
          const preview = new RichTextEngine(import_model.FlowDocument.FromJSON(root));
          try {
    ...
    if (item.Kind === "Insertion") {
      ...
      (0, import_engine_tree.deleteRange)(check, item.Start, item.End);
      if ((0, import_engine_tree.plainText)(check).length !== length - (item.End - item.Start))
        throw new Error(
    ...
    this.Change(() => {
      ...
      try {
        if (item.Kind === "Insertion") {
          ...
          this.mutate((root) => {
            const blocks = (0, import_engine_tree.textBlocks)(root);
            ...
            for (const { block, segment } of locations.reverse()) {
              block.node.children = [
                ...(0, import_engine_tree.sliceInlines)(block.node.children ?? [], 0, segment.Offset),
                ...segment.Inlines.map(import_engine_tree.newIds),
                ...(0, import_engine_tree.sliceInlines)(
                  block.node.children ?? [],
                  segment.Offset,
                  (0, import_engine_tree.inlineText)(block.node).length,
                  true
                )
              ];
            }
  ...
  RejectAllRevisions() {
    const preview = new RichTextEngine(
      import_model.FlowDocument.FromJSON(this.Document.ToJSON())
    );
    try {
  ...
  insertionProperties() {
    const list = (0, import_engine_tree.leaves)(this.Document.ToJSON());
      ...
      if (this.TrackChanges && this.TrackFormatting && !this.reviewSuppressed && changes.length)
        (root.props.Annotations ??= []).push({
          ...
          Data: {
            ...
            CreatedAt: (/* @__PURE__ */ new Date()).toISOString(),
            Operation: operation,
            PropertyChanges: (0, import_engine_tree.clone)(changes)
          }
        });
    });
  }
  ApplyProperty(name, value) {
    ...
    if (this.start === this.end) {
      this.typing[name] = (0, import_engine_tree.clone)(value);
      this.emitSelection();
      return;
    }
    ...
    this.formatMutation(
      "ApplyProperty",
      changes,
      (root) => (0, import_engine_tree.formatRange)(root, this.start, this.end, name, value)
    );
  }
    ...
    this.mutate(
      (root) => {
        ...
        for (const block of this.selectedBlocks(root)) {
          ...
          block.node.children = [
            ...(0, import_engine_tree.sliceInlines)(block.node.children ?? [], 0, from),
            ...strip(
              (0, import_engine_tree.sliceInlines)(block.node.children ?? [], from, to, from > 0)
            ),
            ...(0, import_engine_tree.sliceInlines)(
              block.node.children ?? [],
              to,
  ...
  InsertFragment(nodes) {
    ...
    this.mutate(
      (root) => {
        const from = this.start, to = this.end, oldLength = (0, import_engine_tree.plainText)(root).length;
        (0, import_engine_tree.deleteRange)(root, from, to);
        const block = (0, import_engine_tree.pointBlock)(root, Math.min(from, (0, import_engine_tree.plainText)(root).length));
        const inserted = list.map(import_engine_tree.newIds);
        insertedIds = inserted.map((node) => node.id);
        if (block.node.type !== "Paragraph")
          throw new Error(
            "Select a text paragraph before inserting a fragment."
          );
        const local = Math.max(0, from - block.start), before = (0, import_engine_tree.sliceInlines)(block.node.children ?? [], 0, local), after = (0, import_engine_tree.sliceInlines)(
          block.node.children ?? [],
          local,
          block.text.length,
          true
        ...
        if (allInline) {
          ...
          this.start = this.end = from + inserted.map(import_engine_tree.inlineText).join("").length;
        } else {
          const replacement = [];
          if (inserted[0].type === "Paragraph")
            inserted[0].children = [...before, ...inserted[0].children ?? []];
          else if (before.length)
            replacement.push({ ...(0, import_engine_tree.clone)(block.node), children: before });
          replacement.push(...inserted);
          const tail = inserted.at(-1);
          const tailTextBefore = (0, import_engine_tree.inlineText)(tail).length;
          if (tail.type === "Paragraph")
            tail.children = [...tail.children ?? [], ...after];
          else if (after.length)
            replacement.push(
              local === 0 ? (0, import_engine_tree.clone)(block.node) : (0, import_engine_tree.makeNode)("Paragraph", after, (0, import_engine_tree.clone)(block.node.props))
            );
          block.parent.children.splice(block.index, 1, ...replacement);
          const resultingBlocks = (0, import_engine_tree.textBlocks)(root);
          ...
          this.start = this.end = tailBlock ? tail.type === "Paragraph" ? tailBlock.start + tailTextBefore : tailBlock.end : Math.min(from, (0, import_engine_tree.plainText)(root).length);
        }
        (0, import_engine_tree.mapMetadata)(
          root,
          from,
          to,
          (0, import_engine_tree.plainText)(root).length - oldLength + to - from
        );
      },
      false,
      void 0,
  ...
  InsertEquation(source, format = "latex", displayMode = false) {
    const equation = new import_model.Equation(source, format, displayMode);
  ...
  UpdateEquation(elementId, source, format = "latex", displayMode = false) {
    ...
    const checked = new import_model.Equation(source, format, displayMode);
    this.BeginChange();
    try {
  ...
  SetPageSetup(options) {
    ...
    Object.assign(root.props, (0, import_page_setup.validatePageSetup)(root.props, options));
    this.ReplaceDocument(import_model.FlowDocument.FromJSON(root));
  }
  InsertPageBreak() {
    this.BeginChange();
    try {
  ...
  InsertImage(source, alternativeText = "", width, height) {
    this.InsertNode(
      (0, import_engine_tree.makeNode)("Image", [], {
        Source: source,
  ...
  InsertTable(rows = 2, columns = 2) {
    ...
    this.InsertNode(
      (0, import_engine_tree.makeNode)("Table", [
        (0, import_engine_tree.makeNode)(
          "TableRowGroup",
          Array.from(
            { length: rows },
            () => (0, import_engine_tree.makeNode)(
              ...
              Array.from(
                { length: columns },
                () => (0, import_engine_tree.makeNode)("TableCell", [(0, import_engine_tree.makeNode)("Paragraph")])
              )
            )
          )
        )
  ...
  InsertHyperlink(uri, text = this.Selection.Text || uri) {
    this.InsertNode(
      (0, import_engine_tree.makeNode)("Hyperlink", [(0, import_engine_tree.run)(text, this.insertionProperties())], {
        NavigateUri: uri
      })
    );
  }
  GetSelectedFragment() {
    const root = this.Document.ToJSON(), ranges = new Map((0, import_engine_tree.textBlocks)(root).map((block) => [block.node.id, block]));
    const extract = (item) => {
      const block = ranges.get(item.id);
      if (block) {
        if (this.start === this.end || block.end < this.start || block.start >= this.end)
          return void 0;
        const copy = (0, import_engine_tree.clone)(item);
        if (item.type === "Paragraph")
          copy.children = (0, import_engine_tree.sliceInlines)(
            item.children ?? [],
      ...
      return children.length ? { ...(0, import_engine_tree.clone)(item), children } : void 0;
    };
    return import_model.FlowDocument.FromJSON({
      ...(0, import_engine_tree.clone)(root),
        ...
        for (const block of this.selectedBlocks(root)) {
          ...
          if (this.start === this.end) {
            ...
            const unwrapAt = (items) => items.flatMap((item) => {
              const at = position, length = (0, import_engine_tree.inlineText)(item).length;
            ...
            block.node.children = [
              ...(0, import_engine_tree.sliceInlines)(block.node.children ?? [], 0, start),
              ...unwrap(
                (0, import_engine_tree.sliceInlines)(block.node.children ?? [], start, end, start > 0)
              ),
              ...(0, import_engine_tree.sliceInlines)(
                block.node.children ?? [],
                end,
  ...
  EditFloatingContent(id, action) {
    ...
    const story = new RichTextEngine(
      import_model.FlowDocument.FromJSON(
        (0, import_engine_tree.makeNode)("FlowDocument", (0, import_engine_tree.clone)(target.children ?? []), {
          ...Object.fromEntries(
            Object.entries(target.props).filter(
              ([key]) => INLINE_PROPERTIES.has(key)
            )
          ),
          Annotations: (0, import_engine_tree.clone)(target.props.StoryAnnotations ?? [])
        })
      )
    );
    try {
  ...
  MoveBlocks(ids, parentId, index) {
    ...
    this.mutate(
      (root) => {
        ...
        const selected = (0, import_engine_tree.textBlocks)(root).filter(
      ...
      true,
      void 0,
      { Kind: "Move", Operation: "MoveBlocks", Data: { NodeIds: (0, import_engine_tree.clone)(ids) } }
    );
  }
      ...
      (root) => {
        ...
        for (let y = origin.row; y < origin.row + origin.height; y++) {
          ...
          for (let x = origin.column; x < origin.column + origin.width; x++) {
            ...
            const next = (0, import_engine_tree.makeNode)("TableCell", [(0, import_engine_tree.makeNode)("Paragraph")], {
              ...(0, import_engine_tree.clone)(cell.props)
            });
    ...
    this.mutate(
      (root) => {
        ...
        for (let x = 0; x < grid.width; x++)
          if (!covered.has(x)) {
            const props = (0, import_engine_tree.clone)(
              grid.slots[Math.min(at, grid.rows.length - 1)][x].props
            );
            delete props.RowSpan;
            delete props.ColumnSpan;
            cells.push((0, import_engine_tree.makeNode)("TableCell", [(0, import_engine_tree.makeNode)("Paragraph")], props));
          }
        group.children.splice(
          group.children.indexOf(row) + (before ? 0 : 1),
          0,
          (0, import_engine_tree.makeNode)("TableRow", cells, (0, import_engine_tree.clone)(row.props))
        );
      },
      true,
      void 0,
      ...
      (root) => {
        ...
        grid.rows.forEach((row, y) => {
          ...
          row.children.splice(
            ...
            0,
            (0, import_engine_tree.makeNode)("TableCell", [(0, import_engine_tree.makeNode)("Paragraph")], props)
          );
        });
        if (Array.isArray(table.props.Columns) && table.props.Columns.length)
          table.props.Columns.splice(at, 0, (0, import_engine_tree.makeNode)("TableColumn"));
      },
      true,
      void 0,
    ...
    this.mutate(
      (root) => {
        ...
        parent.children.splice(
          ...
          (0, import_engine_tree.makeNode)(
            "List",
            selected.map((block) => (0, import_engine_tree.makeNode)("ListItem", [block.node])),
  ...
  AddAnnotation(kind, data, start = this.start, end = this.end) {
    ...
    const annotation = {
      ...
      Data: (0, import_engine_tree.clone)(data)
    };
    this.mutate((root) => {
      (root.props.Annotations ??= []).push(annotation);
    });
    return (0, import_engine_tree.clone)(annotation);
  }
  ...
  UpdateAnnotation(id, data) {
    this.mutate((root) => {
      ...
      item.Data = { ...item.Data, ...(0, import_engine_tree.clone)(data) };
    });
  }
  RemoveAnnotation(id) {
    let removed = false;
  ...
  Execute(command, parameter) {
    ...
    switch (name) {
      ...
      case "enterlinebreak":
        return this.InsertNode((0, import_engine_tree.makeNode)("LineBreak"));
      case "delete":
      case "deleteforward":
...
function captureDeletion(root, start, end) {
  const blocks = (0, import_engine_tree.textBlocks)(root).filter(
    (block) => block.end >= start && block.start < end
  ), first = blocks[0];
  const Nodes = blocks.map((block) => ({
    ...(0, import_engine_tree.clone)(block.node),
    children: block.node.type === "Paragraph" ? (0, import_engine_tree.sliceInlines)(
      block.node.children ?? [],
  ...
  const data = {
    Text: (0, import_engine_tree.plainText)(root).slice(start, end),
    Nodes
  };
  if (first && blocks.some((block) => block.parent !== first.parent))
    data.Segments = blocks.filter((block) => block.node.type === "Paragraph").map((block) => {
      ...
      return {
        ...
        Inlines: (0, import_engine_tree.sliceInlines)(block.node.children ?? [], from, to)
      };
    });
  return data;
}
...
function tableContext(root, offset) {
  const block = (0, import_engine_tree.pointBlock)(root, offset), ancestors = findAncestors(root, block.node.id);
...
function replaceTableWithParagraph(root, table) {
  ...
  parent.children.splice(
    ...
    1,
    (0, import_engine_tree.makeNode)("Paragraph")
  );
}
function mapStructuralAnnotations(before, after) {
  if (!Array.isArray(after.props.Annotations) || !after.props.Annotations.length)
    return;
  const oldText = (0, import_engine_tree.plainText)(before), newText = (0, import_engine_tree.plainText)(after);
  ...
  while (suffix < oldText.length - prefix && suffix < newText.length - prefix && oldText[oldText.length - suffix - 1] === newText[newText.length - suffix - 1])
    suffix++;
  const oldBlocks = (0, import_engine_tree.textBlocks)(before), newBlocks = new Map(
    (0, import_engine_tree.textBlocks)(after).map((block) => [block.node.id, block])
  );
...
function setLocalProperty(node, name, value) {
  if (value === void 0) delete node.props[name];
    ...
    Object.defineProperty(node.props, name, {
      value: (0, import_engine_tree.clone)(value),
      writable: true,
      enumerable: true,
      configurable: true
    });
  ...
  return [
    {
      ...
      HadBefore,
      HasAfter,
      ...HadBefore ? { Before: (0, import_engine_tree.clone)(node.props[name]) } : {},
      ...HasAfter ? { After: (0, import_engine_tree.clone)(value) } : {}
    }
  ];
}
function captureInlineProperties(root, start, end, name, value) {
  return (0, import_engine_tree.leaves)(root).filter((leaf) => leaf.end > start && leaf.start < end).flatMap(
...
function validateRevisionProperty(root, change) {
  ...
  const targets = change.Scope === "Node" ? [findNode(root, change.NodeId)] : (0, import_engine_tree.leaves)(root).filter(
    (leaf) => leaf.end > change.Start && leaf.start < change.End
  ).map((leaf) => leaf.node);
  if (change.Scope === "Inline" && (!Number.isInteger(change.Start) || !Number.isInteger(change.End) || change.Start < 0 || change.End < change.Start || change.End > (0, import_engine_tree.plainText)(root).length))
...
function relocateRevisionPatch(root, input, anchors) {
  const patch = (0, import_engine_tree.clone)(input);
  const visit = (change) => {
