...
__export(toolbar_exports, {
  ...
  registerRichTextToolbar: () => registerRichTextToolbar
});
module.exports = __toCommonJS(toolbar_exports);
var import_pagination = require("./pagination.js");
var import_story_session = require("./story-session.js");
...
const home = [
  ...
  { label: "x\xB2", command: "ToggleSuperscript", title: "Superscript" },
...
class RichTextToolbar extends HTMLElementBase {
  ...
  set Editor(value) {
    ...
    if (value) {
      const storyHandler = (event) => {
        ...
        this.executeSafe("EditStory", event.detail.kind);
      };
      ...
      const handler = (event) => {
        ...
        this.executeSafe(
          object?.type === "Figure" || object?.type === "Floater" ? "EditTextBox" : object?.type === "Equation" ? "Equation" : "FloatingLayout"
        );
      };
      const equationKey = (event) => {
        if (!event.defaultPrevented && event.altKey && !event.ctrlKey && !event.metaKey && (event.code === "Equal" || event.key === "=") && !value.IsReadOnly) {
          event.preventDefault();
  ...
  set Mode(value) {
    this.mode = ["home", "insert", "layout", "review", "all"].includes(value) ? value : "all";
    this.render();
  }
  detach() {
  ...
  connectTarget() {
    ...
    this.Editor = candidate instanceof import_control.RichTextBox ? candidate : null;
  }
  Dispose() {
    this.detach();
    this.editor = null;
  ...
  render() {
    ...
    const groups = this.mode === "all" ? [home, insert, layout, review] : [{ home, insert, layout, review }[this.mode]];
  ...
  Refresh() {
    ...
    const locked = !this.editor || this.editor.IsReadOnly, engine = this.editor?.Engine;
    this.shadowRoot.querySelectorAll(".tools button,.tools input,.tools select").forEach((control) => {
      control.disabled = locked && !["PagePreview", "ReviewChanges", "Copy"].includes(
    ...
    if (engine) {
      for (const [command, property, value] of [
        ...
        this.shadowRoot.querySelector(`[data-command="${command}"]`)?.setAttribute(
          "aria-pressed",
          String(engine.Selection.GetPropertyValue(property) === value)
        );
      this.shadowRoot.querySelector('[data-command="TrackChanges"]')?.setAttribute("aria-pressed", String(engine.TrackChanges));
    }
    const status = this.shadowRoot.querySelector(".status");
    if (status) status.textContent = this.status;
  }
  executeSafe(command, parameter) {
    const report = (error) => {
  ...
  Execute(command, parameter) {
    ...
    if (editor.IsReadOnly && !["PagePreview", "ReviewChanges", "Copy"].includes(command))
      return false;
    const engine = editor.Engine, features = new import_document_features.DocumentFeatures(engine);
    const mutate = (action) => {
    ...
    switch (command) {
      ...
      case "Paragraph": {
        ...
        const m = typeof margin === "number" ? { Left: margin, Right: margin, Top: margin, Bottom: margin } : margin && typeof margin === "object" ? margin : { Left: 0, Right: 0, Top: 0, Bottom: 0 };
        this.prompt(
          "Paragraph settings",
          [
            {
            ...
            },
            {
              ...
              value: String(
                Number(paragraph?.LineHeight) > 4 ? paragraph?.LineHeight : (paragraph?.FontSize || 16) * 1.5
              ),
              type: "number"
            },
            {
          ...
          (data) => mutate(() => {
            ...
            if (numeric.some(
              (value) => !Number.isFinite(value) || Math.abs(value) > 2e4
            ) || Number(data.line) <= 0)
      ...
      case "MoveSelection":
        this.prompt(
          ...
          (data) => mutate(() => engine.MoveSelection(Number(data.destination)))
        );
        break;
      ...
      case "TextBox":
        this.prompt(
          ...
          (data) => mutate(() => {
            ...
            if (!Number.isFinite(width) || width < 16 || width > 2e4)
              throw new RangeError(
                "Text box width must be between 16 and 20000 pixels."
              );
            const figure = new import_model.Figure(new import_model.Paragraph(data.text));
            figure.Width = width;
      ...
      case "FloatingLayout": {
        ...
        const dimension = (value, fallback) => typeof value === "number" ? value : value && typeof value === "object" && "Value" in value ? Number(value.Value) || fallback : fallback;
        this.prompt(
          ...
          [
            {
              name: "wrap",
              label: "Text wrapping",
              value: props.WrapStyle || (object.type === "Figure" || object.type === "Floater" ? "Square" : "Inline"),
              options: [
                "Inline",
                "Square",
                "Tight",
      ...
      case "Field":
        this.prompt(
          ...
          (data) => mutate(() => {
            features.InsertField(data.type, data.argument);
      ...
      case "Footer":
        this.editStory(
          editor,
          (0, import_page_setup.pageStoryKey)(
      ...
      case "EvenPageFooter":
        ...
        break;
      case "EditStory":
        this.editStory(editor, parameter);
        break;
      case "PageNumberFooter":
        mutate(() => {
          const p = new import_model.Paragraph(new import_model.Run("Page "));
          p.Inlines.Add((0, import_document_features.createField)("PAGE"));
          p.Inlines.Add(new import_model.Run(" of "));
          p.Inlines.Add((0, import_document_features.createField)("NUMPAGES"));
      ...
      case "PageSetup": {
        ...
        const margins = (0, import_pagination.pageSettings)(props).Padding;
        this.prompt(
          "Page setup",
          [
            {
            ...
            },
            {
              ...
              value: (0, import_page_setup.pageStoryVariantEnabled)(props, "FirstPage") ? "Yes" : "No",
              options: ["No", "Yes"]
            },
            {
              name: "DifferentOddAndEvenPages",
              label: "Different odd and even pages",
              value: (0, import_page_setup.pageStoryVariantEnabled)(props, "EvenPage") ? "Yes" : "No",
              options: ["No", "Yes"]
            }
          ],
          (data) => mutate(
            () => features.SetPageSetup({
              ...
              DifferentOddAndEvenPages: data.DifferentOddAndEvenPages === "Yes"
            })
          )
        );
        break;
      ...
      case "Comment":
        this.prompt(
          ...
          (data) => mutate(() => engine.AddComment(data.text, engine.CurrentAuthor))
        );
        break;
      case "Bookmark":
        this.prompt(
  ...
  prompt(title, fields, submit) {
    const dialog = this.createDialog();
    const targetEditor = this.editor, targetDocument = this.editor?.Document;
    ...
    form.onsubmit = (event) => {
      event.preventDefault();
      if (this.editor !== targetEditor || this.editor?.Document !== targetDocument || this.editor?.IsReadOnly) {
        dialog.close();
        return;
      }
      try {
  ...
  editStory(editor, kind) {
    const session = new import_story_session.DocumentStorySession(editor.Engine, kind);
    const dialog = this.createDialog();
    ...
    hint.textContent = "Formatting, tables, images, fields and equations are preserved. Apply saves one document undo step; Cancel leaves the original story unchanged.";
    const nested = this.ownerDocument.createElement("rich-text-box");
    nested.ViewMode = "continuous";
    nested.style.cssText = "height:300px;min-height:160px;border:1px solid var(--rt-toolbar-border,#ccd5e1)";
  ...
  editEquation(editor) {
    ...
    const originalDocument = editor.Document, originalRevision = editor.Document.Revision;
    const start = editor.Selection.Start.Offset, end = editor.Selection.End.Offset;
    heading.textContent = equation ? "Edit equation" : "Insert equation";
    const workbench = new import_equation_control.EquationEditor();
    workbench.Value = equation ? (0, import_equations.equationOptions)(equation) : {
      Source: editor.Selection.Text.trim() || String.raw`x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}`,
      Format: "latex",
      DisplayMode: false
    };
    const status = this.ownerDocument.createElement("p");
    ...
    apply.onclick = () => {
      try {
        workbench.shadowRoot?.querySelector("textarea")?.dispatchEvent(new Event("change"));
        if (!workbench.Validate()) throw new Error(workbench.Error);
        if (this.editor !== editor || editor.IsReadOnly || editor.Document !== originalDocument)
  ...
  editFloatingStory(editor, object) {
    const dialog = this.createDialog();
    dialog.replaceChildren();
    ...
    nested.Document = import_model.FlowDocument.FromJSON({
      type: "FlowDocument",
    ...
    save.onclick = () => {
      try {
        if (this.editor !== editor || editor.IsReadOnly)
          throw new Error("The target editor changed or became read-only.");
        const find = (node) => node.id === object.id ? node : node.children?.map(find).find(Boolean);
        ...
        editor.Engine.EditFloatingContent(
          object.id,
          (story) => story.ReplaceDocument(nested.Document)
        );
        dialog.close();
        editor.Focus();
      } catch (error) {
  ...
  reviewChanges() {
    ...
    for (const revision of revisions) {
      ...
      const p = this.ownerDocument.createElement("p");
      p.textContent = `${revision.Kind} \xB7 ${revision.Data.Author ?? "Author"}
${revision.Data.Text ?? revision.Data.Operation ?? this.editor.Document.Text.slice(revision.Start, revision.End)}`;
      ...
      for (const action of ["Accept", "Reject"]) {
        ...
        b.onclick = () => {
          ...
          try {
            action === "Accept" ? this.editor.Engine.AcceptRevision(revision.Id) : this.editor.Engine.RejectRevision(revision.Id);
            dialog.close();
            this.reviewChanges();
          } catch (error) {
...
function registerRichTextToolbar(registry = globalThis.customElements) {
