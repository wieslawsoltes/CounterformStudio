...
__export(history_exports, {
  ApplyDocumentPatch: () => ApplyDocumentPatch,
  ApplyPatchToJSON: () => ApplyPatchToJSON,
  CreateDocumentPatch: () => CreateDocumentPatch,
  DocumentObserverError: () => DocumentObserverError,
  InvertDocumentPatch: () => InvertDocumentPatch,
  PatchByteLength: () => PatchByteLength,
...
function ReconcileDocument(document, target) {
  ...
  const normalized = import_model.FlowDocument.FromJSON(input).ToJSON();
  ...
  try {
    ...
    const reconcile = (node) => {
      ...
      if (!element || element.Type !== node.type)
        element = (0, import_model.elementFromJSON)({ ...copy(node), children: [] });
      const before = old.get(node.id)?.props ?? element.ToJSON().props;
      for (const name of /* @__PURE__ */ new Set([
        ...Object.keys(before),
        ...Object.keys(node.props)
      ])) {
        if (name === "Columns" && element instanceof import_model.Table) {
          if (!equal(before.Columns, node.props.Columns)) {
            ...
            const columns = (node.props.Columns ?? []).map(
              (column) => {
                ...
                if (!instance) return (0, import_model.elementFromJSON)(column);
      ...
      if (element instanceof import_model.Run && element.Text !== (node.text ?? "")) {
        const run = element;
        attempt(() => {
          run.Text = node.text ?? "";
        });
