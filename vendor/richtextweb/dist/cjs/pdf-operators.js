...
var pdf_operators_exports = {};
__export(pdf_operators_exports, {
  inspectPDFText: () => inspectPDFText,
  replacePDFSourceText: () => replacePDFSourceText,
  replacePDFTextOperator: () => replacePDFTextOperator
});
module.exports = __toCommonJS(pdf_operators_exports);
var import_pdf_lib = require("pdf-lib");
const identity = () => [1, 0, 0, 1, 0, 0];
const name = import_pdf_lib.PDFName.of;
...
function number(value, fallback) {
  if (value instanceof import_pdf_lib.PDFNumber) return value.asNumber();
  if (fallback !== void 0) return fallback;
  return fail("missing font width or numeric resource value.");
}
function dict(value) {
  return value instanceof import_pdf_lib.PDFDict ? value : void 0;
}
function named(value) {
  return value instanceof import_pdf_lib.PDFName ? value.decodeText() : "";
}
function streamText(stream) {
  let data;
  if (stream instanceof import_pdf_lib.PDFRawStream) {
    data = stream.getContents();
    if (data.length > MAX_BYTES)
      limit("compressed content stream exceeds the 64 MiB limit.");
    const filters = stream.dict.lookup(name("Filter"));
    const filterNames = filters instanceof import_pdf_lib.PDFArray ? filters.asArray() : filters ? [filters] : [];
    const parameters = stream.dict.lookup(name("DecodeParms"));
    for (let index = 0; index < filterNames.length; index++) {
      const stage = import_pdf_lib.PDFDict.withContext(stream.dict.context);
      stage.set(name("Filter"), filterNames[index]);
      const parameter = parameters instanceof import_pdf_lib.PDFArray ? parameters.lookup(index) : parameters;
      if (parameter) stage.set(name("DecodeParms"), parameter);
      const decoder = (0, import_pdf_lib.decodePDFRawStream)(import_pdf_lib.PDFRawStream.of(stage, data));
  ...
  } else if (stream instanceof import_pdf_lib.PDFContentStream)
...
function contextFingerprint(objects, pdf) {
  ...
  while (pending.length) {
    ...
    if (value instanceof import_pdf_lib.PDFRef) {
      ...
      seen.add(value);
      if (value instanceof import_pdf_lib.PDFArray) {
        text(`array:${value.size()};`);
        pending.push(...value.asArray());
      } else if (value instanceof import_pdf_lib.PDFDict) {
        text("dict;");
      ...
      } else if (value instanceof import_pdf_lib.PDFStream) {
        text("stream;");
...
function standard(font) {
  if (!Object.values(import_pdf_lib.StandardFonts).includes(font)) return;
  let result = standardCache.get(font);
  if (!result) {
    result = import_pdf_lib.StandardFontEmbedder.for(
      font
    );
...
function glyphUnicode(glyph) {
  if (!glyphNames.size)
    for (const face of [
      import_pdf_lib.StandardFonts.Helvetica,
      import_pdf_lib.StandardFonts.Symbol,
      import_pdf_lib.StandardFonts.ZapfDingbats
    ]) {
...
function fontCodec(font) {
  ...
  let mapping = toUnicode instanceof import_pdf_lib.PDFStream ? unicodeMap(toUnicode, size) : /* @__PURE__ */ new Map();
  ...
  if (!composite) {
    if (std && (base === import_pdf_lib.StandardFonts.Symbol || base === import_pdf_lib.StandardFonts.ZapfDingbats) && !encodingName) {
    ...
    if (differences instanceof import_pdf_lib.PDFArray) {
      let c = -1;
      for (const item of differences.asArray()) {
        if (item instanceof import_pdf_lib.PDFNumber) {
          c = item.asNumber();
          continue;
        }
        if (!(item instanceof import_pdf_lib.PDFName) || c < 0 || c > 255)
  ...
  if (composite) {
    ...
    const child = descendants instanceof import_pdf_lib.PDFArray ? dict(descendants.lookup(0)) : void 0;
    ...
    if (widths instanceof import_pdf_lib.PDFArray)
      for (let i = 0; i < widths.size(); ) {
        ...
        if (second instanceof import_pdf_lib.PDFArray) {
...
function scan(pdf, indices) {
  ...
  for (const pageIndex of indices) {
    let walk2 = function(object, res, s, path, ancestors, stack) {
      ...
      if (!(stream instanceof import_pdf_lib.PDFStream)) {
      ...
      for (const op of ops) {
        try {
          ...
          if (k === "gs") {
            ...
            if (font instanceof import_pdf_lib.PDFArray) {
          ...
          if (k === "Do") {
            ...
            if (form instanceof import_pdf_lib.PDFStream && named(form.dict.lookup(name("Subtype"))) === "Form") {
              const childState = cloneState(s), matrix = form.dict.lookup(name("Matrix"));
              if (matrix instanceof import_pdf_lib.PDFArray && matrix.size() === 6)
    ...
    const page = pdf.getPage(pageIndex), resources = page.node.Resources() ?? import_pdf_lib.PDFDict.withContext(pdf.context), content = page.node.Contents();
    const original = content instanceof import_pdf_lib.PDFArray ? content.asArray() : content ? [page.node.get(name("Contents"))] : [];
...
function validateReplacement(text, options) {
  ...
  if (options.standardFont && !Object.values(import_pdf_lib.StandardFonts).includes(
...
function setResource(resources, category, key, value) {
  let values = dict(resources.lookup(name(category)))?.clone() ?? import_pdf_lib.PDFDict.withContext(resources.context);
...
function materialize(pdf, frame, resources, retired) {
  ...
  for (const child of frame.children) {
    ...
    if (ref) {
      const xobjects = dict(resources.lookup(name("XObject"))) ?? import_pdf_lib.PDFDict.withContext(pdf.context), key = xobjects.uniqueKey("RichTextForm");
  ...
  if (frame.original instanceof import_pdf_lib.PDFRef) retired.add(frame.original);
...
function removeUnreachableReplacedStreams(pdf, retired) {
  ...
  while (pending.length) {
    ...
    } else if (value instanceof import_pdf_lib.PDFArray) pending.push(...value.asArray());
    else if (value instanceof import_pdf_lib.PDFDict) pending.push(...value.values());
    else if (value instanceof import_pdf_lib.PDFStream) pending.push(value.dict);
  }
...
async function commitEdits(pdf, scanResult, changes, options) {
  ...
  if (options.fontBytes || options.standardFont) {
    ...
    await pdf.flush();
    newRef = font.ref;
    newCodec = fontCodec(pdf.context.lookup(font.ref, import_pdf_lib.PDFDict));
  }
  const prepared = changes.map(({ entry, text, occurrences }) => {
    ...
    if (newRef) {
      const resources = dict(entry.frame.resources.lookup(name("Font"))) ?? import_pdf_lib.PDFDict.withContext(pdf.context);
