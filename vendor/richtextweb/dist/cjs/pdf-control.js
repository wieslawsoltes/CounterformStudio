...
var pdf_control_exports = {};
__export(pdf_control_exports, {
  PDFEditorControl: () => PDFEditorControl,
  registerPDFEditor: () => registerPDFEditor
});
module.exports = __toCommonJS(pdf_control_exports);
...
class PDFEditorControl extends HTMLElementBase {
  ...
  async Load(bytes, options = {}) {
    ...
    const engine = await import_formats_pdf.PDFEditor.Load(bytes);
    const handle = await (0, import_pdf_import.openPDFDocument)(bytes, options);
  ...
  async Refresh() {
    ...
    const handle = await (0, import_pdf_import.openPDFDocument)(bytes, this._importOptions);
    if (this._disposed) {
      await handle.destroy();
      return;
    }
  ...
  async ImportToFlowDocument(options = {}) {
    ...
    const result = await (0, import_pdf_import.extractPDF)(await this.Save(), {
      ...this._importOptions,
      ...options
    });
  ...
  async ExportReflow(options = {}) {
    ...
    return (0, import_formats_pdf.toPDF)(this._flowDocument, {
  ...
  async Find(query, options = {}) {
    ...
    for (let index = 0; index < this.PageCount; index++) {
      ...
      if (!page) {
        page = await (0, import_pdf_import.extractPDFPage)(
  ...
  mutate(action) {
    return this.enqueue(async () => {
      ...
      try {
        ...
        this._engine = await import_formats_pdf.PDFEditor.Load(before);
        await this.Refresh();
        throw error;
      }
  ...
  restore(from, to) {
    return this.enqueue(async () => {
      ...
      this._engine = await import_formats_pdf.PDFEditor.Load(bytes);
      await this.Refresh();
      from.pop();
      to.push(current);
      this.updateToolbar();
  ...
  async render() {
    ...
    try {
      ...
      if (version !== this._renderVersion || this._disposed) return;
      this._textLayer = new import_pdf_import.pdfjs.TextLayer({
...
function registerPDFEditor(registry = globalThis.customElements) {
  (0, import_control.registerRichTextWeb)(registry);
