...
var collaboration_document_exports = {};
__export(collaboration_document_exports, {
  CollaborativeDocumentSession: () => CollaborativeDocumentSession,
  RichDocumentBinding: () => RichDocumentBinding
});
module.exports = __toCommonJS(collaboration_document_exports);
var import_model = require("./model.js");
var import_engine_tree = require("./engine-tree.js");
var import_collaboration = require("./collaboration.js");
...
function jsonValue(value, depth = 0) {
  if (depth > 256)
    throw new import_collaboration.CollaborationConflictError("JSON nesting limit exceeded.");
  ...
  if (!value || typeof value !== "object" || !Array.isArray(value) && Object.getPrototypeOf(value) !== Object.prototype && Object.getPrototypeOf(value) !== null)
    throw new import_collaboration.CollaborationConflictError(
      "Collaboration values must be finite JSON data."
    );
  for (const [key, item] of Object.entries(value)) {
    if (["__proto__", "constructor", "prototype"].includes(key))
      throw new import_collaboration.CollaborationConflictError("Reserved JSON property name.");
    jsonValue(item, depth + 1);
  }
}
function normalizeDocument(document) {
  const root = copy(
    document instanceof import_model.FlowDocument ? document.ToJSON() : document
  );
  jsonValue(root);
  const walk = (node) => {
    if (node.type === "Run")
  ...
  walk(root);
  return import_model.FlowDocument.FromJSON(root).ToJSON();
}
...
class CollaborativeDocumentSession {
  DocumentId;
  ActorId;
  OperationGenerated = new import_model.EventDispatcher();
  Changed = new import_model.EventDispatcher();
  Conflict = new import_model.EventDispatcher();
  epoch;
  initial;
  initialHash;
  checkpoint;
  ...
  get Document() {
    return import_model.FlowDocument.FromJSON(this.document);
  }
  get DocumentJSON() {
    return copy(this.document);
  }
  get Text() {
    return (0, import_engine_tree.plainText)(this.document);
  }
  get VersionVector() {
  ...
  textSession(node) {
    return new import_collaboration.CollaborativeTextSession({
    ...
    for (const [name, value] of Object.entries(node.props ?? {})) {
      ...
      if (name === "Annotations") {
        if (!Array.isArray(value))
          throw new import_collaboration.CollaborationConflictError("Annotations must be an array.");
  ...
  applyAction(state, operation, action, index) {
    ...
    if (!action || typeof action !== "object" || !allowed[action.Kind] || Object.keys(action).some((key) => !allowed[action.Kind].includes(key)) || "Remove" in action && typeof action.Remove !== "boolean" || "Remove" in action && action.Remove === true && "Value" in action)
      throw new import_collaboration.CollaborationConflictError("Malformed rich action fields.");
    const stamp = {
    ...
    switch (action.Kind) {
      case "Insert": {
        ...
        (0, import_model.elementFromJSON)(input);
      ...
      case "Property": {
        if (!safeId(action.Name) || [
          ...
          throw new import_collaboration.CollaborationConflictError("Invalid property transaction.");
        const target = mutable(action.NodeId), old = target.Properties.get(action.Name);
        if (!action.Remove)
          (0, import_model.elementFromJSON)({
            type: target.Type,
            id: target.Id,
  ...
  accept(operation, remote) {
    ...
    import_model.FlowDocument.FromJSON(document);
    this.state = state;
  ...
  InsertNode(parentId, index, node, collection = "Children") {
    ...
    jsonValue(node);
    (0, import_model.elementFromJSON)(node);
    insert(
      node,
      parentId,
  ...
  UpdateDocument(document) {
    ...
    for (const [id, item] of wanted) {
      ...
      if (entries.size !== (item.Node.props.Annotations?.length ?? 0))
        throw new import_collaboration.CollaborationConflictError("Annotation IDs must be unique.");
  ...
  static FromSnapshot(snapshot, actorId) {
    if (!snapshot || snapshot.Protocol !== 2 || !safeId(snapshot.Epoch) || !Array.isArray(snapshot.Operations))
      throw new import_collaboration.CollaborationConflictError("Malformed rich document snapshot.");
    jsonValue(snapshot);
    ...
    this.subscriptions.push(
      Session.Changed.Subscribe(({ Remote, Checkpoint }) => {
        ...
        if (this.applying || this.disposed || equal(
          ...
          return;
        this.applying = true;
        const selection = Engine.CaptureSelectionState(), start = new import_model.TextPointer(Engine.Document, selection.Start), end = new import_model.TextPointer(Engine.Document, selection.End);
        try {
          const engine = Engine;
