...
__export(collaboration_exports, {
  ...
  CollaborationConflictError: () => CollaborationConflictError,
  CollaborativeTextSession: () => CollaborativeTextSession
});
...
__reExport(collaboration_exports, require("./collaboration-document.js"), module.exports);
...
const defaultDocument = new import_model.FlowDocument();
...
function captureFormatting(engine) {
  return (0, import_engine_tree.leaves)(engine.Document.ToJSON()).filter((leaf) => leaf.end > leaf.start).map((leaf) => ({
    Start: leaf.start,
    End: leaf.end,
    Properties: Object.fromEntries(
      [...properties].map((name) => [
