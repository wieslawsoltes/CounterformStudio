...
def shape(raw, text, lang='en'):
    b=blob(raw,len(raw),0,None,None); f=face(b,0); ft=font(f); buf=buffer()
    try:
        ...
        features=(Feature*5)(*(Feature(int.from_bytes(tag.encode(),'big'),1,0,0xffffffff) for tag in ['salt','calt','ccmp','kern','liga']))
...
checks=[]
with TemporaryDirectory(prefix='counterform-context-') as temp:
    tmp=Path(temp);subprocess.run(['node','tests/contextual-fixtures.mjs',str(tmp)],cwd=root,check=True)
    for name,fea,text in json.loads((tmp/'cases.json').read_text()):
        expected=TTFont(tmp/'base.ttf');addOpenTypeFeaturesFromString(expected,fea);stream=BytesIO();expected.save(stream)
        raw=(tmp/(name+'.ttf')).read_bytes()
        actual=TTFont(BytesIO(raw));actual.ensureDecompiled()
        for lang in (['en','tr'] if name=='language' else ['en']):
            got=shape(raw,text,lang);want=shape(stream.getvalue(),text,lang)
            assert got==want,(name,lang,got,want)
        checks.append(name);print('PASS',name,': glyphs, clusters, advances and offsets match fontTools/HarfBuzz')
(root/'test-results').mkdir(exist_ok=True)
(root/'test-results/contextual-report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'harfbuzz':version,'oracle':'fontTools FEA compilation + native HarfBuzz'},indent=2)+'\n')
