...
checks=[]
with TemporaryDirectory(prefix='counterform-attachments-') as temp:
    tmp=Path(temp);subprocess.run(['node','tests/attachment-fixtures.mjs',str(tmp)],cwd=root,check=True)
    for name,fea,text in json.loads((tmp/'cases.json').read_text()):
        ...
        for ext in ['ttf', 'otf']:
            raw=(tmp/(name+'.'+ext)).read_bytes()
            actual=TTFont(BytesIO(raw));actual.ensureDecompiled()
            got=shape(raw,text);want=shape(stream.getvalue(),text)
            assert got==want,(name,ext,got,want)
        checks.append(name);print('PASS',name,': glyphs, clusters, advances and offsets match fontTools/HarfBuzz')
(root/'test-results').mkdir(exist_ok=True)
(root/'test-results/attachments-report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'harfbuzz':version,'oracle':'fontTools FEA compilation + native HarfBuzz'},indent=2)+'\n')
